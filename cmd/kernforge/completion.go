...
var slashSubcommandDescriptions = map[string]map[string]string{
	...
	"goal": {
		"--run":           "Record a goal and immediately start the autonomous loop.",
		"--no-run":        "Record a goal and write .kernforge/goals/latest artifacts without running automation.",
		"--file":          "Load a goal objective from a markdown file and record it.",
		"@GOAL.md":        "Load a goal objective from a markdown file and record it.",
	...
	"fuzz-func": {
		...
		"continue": "Approve a pending recovered build configuration. Optional --profile smoke|extended|repro|minimize switches the run profile.",
		"repro":    "Re-run a saved fuzz plan in repro profile against a specific crash input (single-shot replay).",
		"minimize": "Re-run a saved fuzz plan with libFuzzer crash minimization (-minimize_crash=1) against a specific crash input.",
