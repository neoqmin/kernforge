...
func formatReviewerGateUnavailableToolError(cfg Config, run ReviewRun) string {
	...
	if reviewRunHasReviewerTimeoutFailure(&run) {
		if korean {
			hint = " 다음 reviewer call은 timeout을 자동으로 한 단계 늘립니다; `/model cross-review` 또는 `/model clear cross-review`로 복구할 수 있습니다."
		} else {
			hint = " The next reviewer call will automatically extend the timeout once; recovery options are `/model cross-review` or `/model clear cross-review`."
		}
	}
	if korean {
