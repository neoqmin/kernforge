...
func renderLiveProviderDrillMarkdown(report *LiveProviderDrillReport) string {
	...
	for _, turn := range copyReport.Turns {
		fmt.Fprintf(&b, "- `%s` class=`%s` kind=`%s` route=`%s` quality=`%s` gate=`%s` latency_ms=`%d` retries=`%d` malformed=`%d` reviewer_separated=`%t`\n",
			turn.TurnID,
			turn.RequestClass,
			turn.LifecycleKind,
			turn.RouteMode,
	...
	if len(copyReport.RouteHealthEvents) > 0 {
		...
		for _, event := range copyReport.RouteHealthEvents {
			fmt.Fprintf(&b, "- `%s` role=`%s` class=`%s` status=`%s` provider=`%s` model=`%s` retry_count=`%d` malformed=`%d` recommendation=%s\n",
