...
반복적인 TavernKernel 재분석 테스트에서 드러난 핵심 문제는 "LLM worker/reviewer/synthesis가 각자 산출물을 해석하면서 같은 source anchor와 flow rule을 다르게 재라벨링할 수 있다"는 점이었다. 따라서 프롬프트 보정 중심의 접근을 줄이고, 분석 런타임이 먼저 deterministic architecture fact pack을 만든 뒤 모든 LLM 단계가 같은 사실 집합을 참조하도록 구조를 바꿨다.
...
4. 드라이버/보안 프로젝트 일반화
   - TavernKernel 문자열에 의존하지 않고 `DriverEntry`, WDM/driver build context, IRP/IOCTL, `ObRegisterCallbacks`, process notify, minifilter, memory/handle/security overlay 같은 일반 Windows driver 패턴으로 동작한다.
...
TavernKernel 드라이버 프로젝트 답변 분석 결과를 기준으로 추가 보강을 진행했다. 기존 답변은 cached analysis를 사용하긴 했지만, 드라이버를 DLL처럼 표현할 위험, `TavernKernelAPI.cpp`를 외부 API entrypoint처럼 오해할 위험, user-mode IOCTL wrapper와 kernel-side dispatch/validation을 섞는 위험, 이전 persistent memory의 stale caveat를 최신 분석보다 우선하는 위험이 있었다.
...
2. Domain-specific critical anchors
   - `DriverEntry`, `TavernKernelCore::Initialize`, control device creation, kernel IOCTL dispatcher, request origin validation, command validation, IOCTL payload decryption, object callback registration/pre-callback, process monitor, dynamic kernel API resolver, policy engine, user-mode IOCTL client를 역할별 anchor로 선별한다.
...
3. Driver-specific flow map
   ...
   - user-mode `TavernKernelManager` wrapper와 kernel-side `TavernKernelCore` dispatch/validation을 분리해서 설명하라는 규칙을 포함했다.
...
7. 회귀 테스트
   - TavernKernel-like driver fixture를 추가해 driver 구조 질문에서 kernel critical anchor가 answer pack과 렌더링 결과에 모두 포함되는지 검증한다.
...
8. Review diagnostics 표시 분리
   ...
   - provider review timeout이 발생한 TavernKernel 같은 케이스가 산출물 품질 거절처럼 보이지 않도록 했다.

9. TavernKernel 구조 QA 2차 피드백 반영
   - 테스트 답변은 전반적으로 개선됐다. `.sys` driver, user-mode manager, kernel-side IOCTL dispatch, critical source anchors를 구분해 설명했다.
   - 남은 문제는 answer pack의 domain flow가 anchor를 너무 직선적인 call chain처럼 제공해 모델이 `ValidateRequestorIsTavern`을 `DeviceIoControlIrpHandleRoutine` 내부 호출처럼 설명하거나, `Finalize`를 runtime enforcement path처럼 연결할 수 있다는 점이었다.
   ...
   - `ValidateRequestorIsTavern` 설명을 control-channel request origin validation으로 보정하고, command payload validation과 직접 연결하지 말라는 evidence rule을 추가했다.
   - `TavernKernelAPI::Initialize`/`GetExportFunctionAddress`가 dynamic kernel API resolver 대표 anchor로 우선되도록 보정해 `_GetEnclosingSectionHeader` 같은 helper가 대표 흐름으로 과대표현되지 않게 했다.
   - `new_primary_scope`는 stale/invalidation이 아니라 cache scope marker이므로 deep QA stale marker에서 제외했다.

10. TavernKernel 구조 QA 3차 피드백 반영
   ...
   - 남은 문제는 `Key Source Anchors` 표에서 `TavernKernelCore.cpp:1285` (`GetControlPid`)를 `Finalize`처럼 재라벨링한 점이었다. 이는 source-only anchor와 lifecycle 용어가 섞이면서 발생한 라벨링 오류다.
   ...
   - `TavernKernelCore::UnloadRoutine`을 `driver_unload_entry`, `TavernKernelCore::Finalize`를 `teardown_cleanup`으로 별도 critical anchor에 추가했다.
   - `TavernKernelProcessMonitor::StartProcessMonitor`가 `process_monitor` 대표 anchor가 되도록 하고, `TavernKernelAPI::TavernPsSetCreateProcessNotifyRoutineEx`는 `process_notify_api_wrapper`로 분리했다.
   - `TavernKernelFileFilter::Initialize`를 `file_minifilter` 대표 anchor로 우선해 임의 callback/helper line이 파일 필터 대표처럼 출력되지 않도록 했다.
   - critical anchor retrieval 한도를 18개로 늘려 driver 답변에 teardown/process monitor/file filter/API wrapper까지 함께 들어가게 했다.

11. TavernKernel 구조 QA 4차 피드백 반영
   - 새 답변에서도 구조 흐름은 좋아졌지만, `TavernKernelCore::Initialize`/`DeviceIoControlIrpHandleRoutine`/`Finalize`의 line number가 다시 섞였다.
...
12. TavernKernel 구조 QA 5차 피드백 반영
   - 최신 답변은 line anchor 라벨링은 안정화됐지만, `StartObjectFilter`를 DriverEntry/Core Initialize 흐름에 넣고 `ValidateRequestorIsTavern`을 `DeviceIoControlIrpHandleRoutine` 내부 command spine처럼 설명하는 문제가 남았다.
   ...
   - `ValidateRequestorIsTavern`은 `IRP_MJ_CREATE`/control-open validation spine에 두고, `DeviceIoControl` command spine에서는 `DecryptIoctlData`, per-command size/shape check, `IsValidCommand`, `GetControlPid`/`requestorPid` 기반 검사로 분리하도록 했다.
   ...
   - `TavernKernelObjectFilter::Initialize`를 `object_filter_initialization`, `TavernKernelProcessMonitor::Initialize`를 `process_monitor_initialization` critical anchor로 추가해 initialization과 runtime callback registration을 구분했다.
   - fixture에 `GetControlPid`, object/process monitor Initialize anchor와 관련 call edge를 추가하고, load/init spine에 `StartObjectFilter`가 들어가지 않는지와 DeviceIoControl command spine에 `ValidateRequestorIsTavern` 직접 단계가 들어가지 않는지 회귀 테스트를 추가했다.
...
13. TavernKernel 구조 QA 6차 피드백 반영
   - 최신 답변은 `StartObjectFilter`/`ValidateRequestorIsTavern`의 위치는 바로잡았지만, `DeviceIoControlIrpHandleRoutine` 내부 command spine을 `DeviceIoControl 핸들러 -> IOCTL 명령별 디스패치` 수준으로 너무 얇게 요약했다.
   ...
   - `Common/`을 `TavernKernel/` 하위처럼 렌더링할 수 있는 파일 트리 혼동을 줄이기 위해 answer pack 초반에 `Root folder map (exact sibling paths)`를 추가했다.
   - folder placement rule을 추가해 slash-separated path를 그대로 사용하고, `Common`은 실제 경로가 `TavernKernel/Common`이라고 나올 때만 드라이버 하위로 중첩하라고 명시했다.

14. TavernKernel 구조 QA 7차 피드백 반영
   - 최신 답변은 루트 폴더 sibling 규칙은 잘 반영했지만, `TavernKernelAPI.h/`, `TavernKernelCore.h/`, `TavernKernelProcessMonitor.h/` 같은 header file을 top-level directory처럼 추가했다.
...
15. Windows driver 범용화 보정
   - TavernKernel은 회귀 fixture와 실전 품질 확인 대상일 뿐, production answer pack/fast-path 규칙이 특정 프로젝트명이나 특정 심볼명에 묶이면 안 된다.
   - production 코드의 driver QA 규칙에서 `TavernKernelManager`, `TavernKernelCore`, `StartObjectFilter`, `ValidateRequestorIsTavern`, `GetControlPid` 같은 전용 문구를 제거했다.
   ...
   - critical anchor classifier도 `TavernKernel*` prefix가 아니라 `objectfilter::initialize`, `processmonitor::initialize`, `PsSetCreateProcessNotifyRoutine`, `filefilter::initialize`, `kernel API resolver`, `controlPid/requestorPid/callerPid` 같은 드라이버 공통 패턴으로 동작하도록 바꿨다.
   - `analysis_qa_context.go`와 `analysis_context.go`에 TavernKernel 전용 문자열이 남지 않는 것을 확인했다. TavernKernel 문자열은 fixture/test data와 실제 검증 기록 문서에만 남긴다.
...
16. Windows driver 폴더/흐름 산출물 품질 보정
   ...
   - 위 보정은 TavernKernel 전용 문자열 없이 적용된다. production Go code에는 `tavern` 문자열이 남지 않는 것을 case-insensitive 검색으로 확인했다.
   - 추가 회귀 테스트:
...
18. 재분석 산출물 20260428-143105 리뷰 반영
   ...
   - 남은 원재료 품질 이슈:
     - `latest/docs/FOLDER_MAP.md`에서 `TavernKernel/` 책임이 user-mode harness로 뒤집혔다.
...
19. 재분석 산출물 20260428-151901 리뷰 반영
   ...
   - 남은 원재료 품질 이슈:
     - `latest/docs/FOLDER_MAP.md`에서 `TavernKernelTestConsole/`이 user-mode driver control harness가 아니라 build/release/packaging tooling으로 오분류됐다.
...
20. 재분석 산출물 20260428-160336 리뷰 반영
   - 새 산출물은 `TavernKernelTestConsole/` 책임과 `DEVELOPER_OVERVIEW.md` Project Shape가 의도대로 개선됐다. user-mode harness/control root가 `TavernKernelTestConsole/`로 잡히고, kernel driver root와 shared contract root도 올바르게 분리됐다.
   - 남은 품질 이슈:
...
21. 재분석 산출물 20260428-171116 리뷰 반영
   - 새 산출물은 root `.`, kernel/runtime driver entry file, Project Shape의 driver/harness 분리가 개선됐다. `Kernel/runtime driver entry files`에는 `TavernKernel/TavernKernel.cpp`만 남았고, solution startup executable과 kernel DriverEntry가 분리됐다.
   - 남은 품질 이슈:
     - `Common/` 폴더가 `TavernKernelTestConsole` 관련 worker 문구에 오염되어 shared contract/common utility가 아니라 user-mode bootstrap/service lifecycle harness로 오분류됐다.
...
22. 반복 테스트 루프를 줄이기 위한 근본 보정
   - 문제:
     ...
     - `TavernKernelCore.cpp`처럼 경로 없는 bare filename이 worker report에서 나오면 실제 snapshot 위치로 resolve되지 못하고 root `.` record를 오염시켰다.
...
23. 재분석 산출물 20260428-183119 리뷰 반영
   - 좋아진 점:
     - `FOLDER_MAP.md`에서 `TavernKernel/`, `TavernKernelTestConsole/`, `Common/`, root `.`, `BuildCab/`, `Batch/`, `VMProtect/` 책임이 안정화됐다.
   ...
   - 남은 품질 이슈:
     ...
     - ObjectFilter activation 설명에서 `TavernKernelObjectFilter::Initialize -> ObRegisterCallbacks`처럼 상태 초기화와 runtime registration이 다시 섞였다. Domain Critical Anchors에는 `object_filter_initialization`과 `object_callback_registration`이 분리되어 있었으므로 최종 합성 단계의 guardrail이 부족한 문제다.
   - 보정:
...
실제 TavernKernel 기존 산출물 확인:

1. `C:\git\im-tavern-client\.kernforge\analysis\latest\structural_index_v2.json`에는 `DriverEntry`, `TavernKernelCore::Initialize`, `TavernKernelCore::DeviceIoControlIrpHandleRoutine`, `TavernKernelCore::ValidateRequestorIsTavern`, `TavernKernelCore::DecryptIoctlData`, `TavernKernelObjectFilter::StartObjectFilter`, `TavernKernelObjectFilter::*ObjectPreCallback`, `TavernKernelAPI::TavernMmCopyVirtualMemory`가 이미 기록되어 있다.
...
실제 TavernKernel 재분석 결과 확인:

1. 대상 결과: `C:\git\im-tavern-client\.kernforge\analysis\20260428-091058_map_map_the_architecture,_subsystems,_ownership,_mod.md`
2. 최종 보고서 초반에서 `TavernKernel`을 Windows kernel-mode driver, output `tvk.sys`로 설명했고, `TavernKernelTestConsole`을 user-mode development harness로 분리했다.
3. `DriverEntry`, `TavernKernelCore::Initialize`, `DeviceIoControlIrpHandleRoutine`, `ValidateRequestorIsTavern`, `TavernKernelAPI::Initialize`, object/process/file filter 흐름이 구조 설명에 포함됐다.
...
5. 남은 품질 이슈도 발견했다.
   ...
   - `TavernKernel` 폴더가 `DeviceIoControl` 단어 때문에 user-mode harness로 잘못 분류될 수 있었다.
6. 위 이슈는 코드 보정 완료.
   ...
   - folder/module responsibility classifier에서 `Common`, `TavernKernelTestConsole`, kernel driver folder를 더 명확히 분리했다.
   ...
   - TavernKernel-like fixture 회귀 테스트를 추가했고 `go test -count=1 ./...`, `go build ./...`, `go vet ./...`를 통과했다.
...
1. 필수 구현 항목은 현재 없다.
2. 실제 TavernKernel 같은 대형 driver 프로젝트에서 최신 바이너리로 `/analyze-project --mode map`을 다시 실행한 뒤, "프로젝트 전체 구조", "초기화 흐름", "IOCTL dispatch/validation", "object filter enforcement" 질문을 반복해 실제 모델 출력 품질을 확인해야 한다.
