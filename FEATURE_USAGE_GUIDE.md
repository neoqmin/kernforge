...
Changing the main provider/model preserves explicit analysis worker and reviewer profiles. Use `/model analysis clear` when you want project analysis to inherit the current main model again instead of a previously dedicated route, or use `/model analysis-worker 0` / `/model analysis-reviewer 0` to reset only one role. In the interactive provider picker for analysis worker/reviewer, choose `0` to do the same reset.
