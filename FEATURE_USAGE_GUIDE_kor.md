...
2. 새 request-class-aware orchestration은 사용자 요청을 `review_only`, `document_artifact`, `review_then_modify`, `modify_then_review`, `verification_only`, `validation_only`, `general`로 분류한다. 선택된 class와 이유는 `ReviewRun`, `RuntimeGateLedger`, status surface, artifact, MCP review response에 남는다.
...
4. cross-model 모드에서는 primary 구현 책임과 독립 reviewer 피드백을 분리한다. actionable reviewer item은 `cross_review_triage`에서 `accepted_fixed`, `accepted_deferred`, `rejected_with_reason`, `needs_user_decision` 중 하나가 되어야 하며, 수정 근거, 보류 이유, 기술적 기각 이유, 사용자 결정 guidance가 부족하면 obligation으로 남는다.
5. document-artifact 요청은 artifact-quality check를 1차 gate로 사용한다. 요청한 artifact 존재, 요청 topic coverage, placeholder/TODO-only 여부, unsupported shell/code verification claim 여부를 검사한다. 그래서 generated-document skip behavior를 보존하고 irrelevant code review나 shell verification loop로 과차단하지 않는다.
6. UX/Ops layer는 이 lifecycle 결정을 `/status`, `/hooks`, session dashboard, Markdown artifact, JSON artifact, MCP response로 노출한다. compact status에는 latest review id, request class, lifecycle phase, trigger/target/mode, route mode, review/repair/document/verification gate status, gate verdict/action, second-pass state, triage status별 count, incomplete triage blocker, 남은 obligation, final-answer correction 상태, blocker class, 다음 추천 명령이 들어간다.
7. blocker class는 code repair blocker, reviewer route problem, evidence gap, verification gap, document-artifact quality blocker, final-answer completeness blocker를 분리한다. 그래서 cross-review residual-risk noise가 primary code blocker를 가리지 않는다.
8. cross-review triage artifact는 각 finding에 id/title, location, status, reason, required fix, fix refs/changed paths, verification refs, user-action guidance를 보여준다. `needs_user_decision`은 raw review 문장에서 다음 행동을 추측하게 하지 않고 구체적인 continuation prompt를 제공한다.
9. final-answer completeness correction은 changed-file disclosure, review/self-review disclosure, validation disclosure, remaining-risk disclosure, review-only findings-first/no-edit formatting, document-artifact validation disclosure 같은 lifecycle fact로 노출한다. 내부 prompt text는 노출하지 않는다.
10. 알려진 한계: raw model output은 local artifact ref 뒤에 남기고 MCP response에 dump하지 않는다. MCP field는 additive/backward-compatible로 유지한다. generated-document artifact skip behavior는 code review gate와 계속 분리한다. 전체 패키지 테스트는 한 번에 빠르게 끝난다고 가정하지 말고 cluster 단위 진단이 필요할 수 있다.
