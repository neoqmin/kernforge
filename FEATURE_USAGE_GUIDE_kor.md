...
18. Claude Code CLI 기본 선택지는 현재 Claude family version을 표시하지만, 실제 CLI 실행에는 `sonnet`, `opus`, `haiku` 같은 안전한 alias를 넘긴다. `/review`, 자연어 리뷰, 수정 전 repair check는 main-first로 동작한다. active main model이 로컬 evidence로 첫 구조화 리뷰를 만들고, optional cross route가 설정돼 있으면 같은 evidence와 primary draft를 받아 second-pass로 다시 본다. cross route가 없으면 단일 모델 모드로 명시하고, 최신 사용자 요청을 review-only, bug finding, targeted modification, implementation plus verification, review-after-modification, documentation/status update, commit-ready cleanup 중 하나로 먼저 분류한 뒤 context discovery, 구현/리뷰, touched code self-review, validation, final response 순서의 staged loop를 prompt contract로 주입한다. domain 전문성은 `security`, `design`, `false_positive`, `regression`, `test`, `final_gate` lens로 prompt에 주입된다. cross reviewer가 실패하거나 빈 응답을 반환하거나 `weak` 품질로 끝나도 run은 degraded 상태로 표시될 뿐, main review finding 보고나 repair loop 시작 자체를 막지는 않는다. cross reviewer finding이 repair plan에 들어가면 implementation model은 이를 독립 리뷰 피드백으로 보고 accepted/fixed, accepted/deferred, rejected_with_reason, needs_user_decision 중 하나로 판단해 최종 응답이나 수리 근거에 남겨야 한다.
