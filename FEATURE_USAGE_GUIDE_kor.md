...
27. 수정 또는 로컬 리뷰 final answer가 사용자에게 보이기 전에 pre-final coding harness가 완료 사실을 검사한다. 수정 흐름은 변경 파일 또는 무변경 명시, review/self-review 결과, validation 결과 또는 미실행 이유, remaining risk 또는 known blocker 없음이 필요하다. review-only 흐름은 read-only 상태를 유지하고 findings-first로 시작해야 하며, finding이 없으면 남은 test/evidence risk를 말해야 한다. generated document artifact와 단순 status/command 턴은 skip rule로 과차단하지 않는다.
