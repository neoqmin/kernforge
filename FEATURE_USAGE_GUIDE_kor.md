...
생성 문서 세트에는 run 마지막에 출력된 assistant-facing final synthesis를 그대로 보존하는 `FINAL_REPORT.md`와 architecture, security, entrypoint, build artifact, coverage ledger, evidence packet, verification, fuzz target, operation 운영 문서가 함께 들어간다.
dashboard는 이 문서들을 inline Markdown viewer로 열며, 최종 리포트나 다른 generated document가 기본 패널에서 읽기 길면 `Reader` 버튼으로 full-window reading mode를 사용할 수 있다. 상단 metric에는 skipped file 수와 evidence packet 수가 함께 표시된다.
