...
9. progress 출력도 같은 phase 언어를 쓴다. 긴 대기 중에는 Kernforge가 primary model, cross reviewer, local tool, verification command, final-answer correction 중 무엇을 기다리는지 말하고, gate가 실제 통과하기 전에는 완료처럼 보이는 문구를 찍지 않는다.
10. blocker class는 `code_repair_blocker`, `reviewer_route_problem`, `evidence_gap`, `verification_gap`, `document_artifact_quality`, `final_answer_contract`, `user_decision_required`로 분리한다. 그래서 cross-review route warning이나 document artifact detail이 primary code blocker를 가리지 않는다.
11. cross-review triage artifact는 각 finding에 id/title, reviewer route, severity, location, status, reason, required fix, fix evidence, verification evidence, safe continuation guidance를 보여준다. `needs_user_decision`은 raw review 문장에서 다음 행동을 추측하게 하지 않고 확인할 대상, 안전하게 바꿀 범위, 아직 바꾸면 안 되는 범위, 다음 명령을 제공한다.
12. single-model mode는 degraded fallback이 아니다. status/report는 `single_model_second_pass_ran`, `single_model_second_pass_cached`, `single_model_second_pass_skipped`, `cross_model_review_ran`, `reviewer_only_post_change_review_used`를 구분하며 single-model second pass를 독립 cross-review처럼 설명하지 않는다.
13. document-artifact flow는 artifact path, artifact-quality status, 필요한 경우 source-review evidence status, verification skip 또는 required reason, 남은 limitation을 보여준다. document artifact final-answer correction은 code repair가 아니라 final-answer contract correction으로 표시한다.
14. final-answer completeness correction은 changed-file disclosure, review/self-review disclosure, validation disclosure, remaining-risk disclosure, review-only findings-first/no-edit formatting, document-artifact path/quality/verification/limitation disclosure 같은 lifecycle fact로 노출한다. 내부 prompt text는 노출하지 않고, "완료", "패치됨", "구현됨" 같은 generic legacy 완료 문구만으로는 evidence를 충족하지 않는다.
15. MCP operator-card field는 additive/backward-compatible이다. `lifecycle_timeline`, `compact_status`, `blocker_summary`, `route_quality`, `final_answer_contract_status`, `next_recommended_command`를 제공하므로 MCP consumer는 prose나 raw model output을 파싱하지 않고 operator card를 그릴 수 있다.
16. 알려진 한계: raw model output은 local artifact ref 뒤에 남기고 MCP response에 dump하지 않는다. MCP field는 additive/backward-compatible로 유지한다. generated-document artifact skip behavior는 code review gate와 계속 분리한다. ambiguity가 높은 자연어 분류는 보수적으로 남긴다. 전체 패키지 테스트는 한 번에 빠르게 끝난다고 가정하지 말고 cluster 단위 진단이 필요할 수 있다.
