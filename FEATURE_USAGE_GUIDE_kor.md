...
11. `progress_display`가 진행 표시 방식을 제어하며 기본값은 일반 review/coding 작업을 읽기 쉽게 유지하는 `compact`이다. `/progress-display auto|compact|stream`으로 REPL에서 바로 바꾼다. `compact`는 짧은 progress를 footer 중심으로 보여주고, `auto`는 반복적인 상세 review flow text 없이 durable tool/model/route와 project analysis event를 남기며, `stream`은 상세 디버깅용으로 모든 update를 지속 기록한다.
...
- `/goal start --run "missing recovery test와 docs를 추가해"`는 goal을 기록한 뒤 즉시 autonomous loop를 실행한다.
- `/goal start @GOAL.md`는 markdown goal을 기록하되 실행하지 않는다.
...
7. goal 실행 중에는 write, diff preview, shell, git approval을 session 안에서 bypass해서 해당 확인으로 멈추지 않는다. 다만 암시적 model-backed review gate는 `review.model_review_consent`에 따라 `Run model review now? [y/N/a=auto-review for this session]`를 물을 수 있다.
...
9. `/progress-display compact`는 일반 operator 경험의 기본값이다. review progress는 `review 1/6 scope`, `review 2/6 evidence`, `main ... done`, `cross ... done`, `gate ...`, `next ...` 같은 짧은 action line으로 보이고, 반복되는 전체 1->6 flow text와 `phase/waiting_on/next` 진단 suffix는 출력하지 않는다. `/progress-display auto`도 정상 interactive review에서는 같은 긴 flow spam을 피한다.
10. `/progress-display stream`은 상세 디버깅 모드다. stream 모드에서는 기존처럼 phase, lifecycle, waiting target, next transition, 전체 flow text를 transcript에 남긴다. `verbose` config alias는 계속 `stream`으로 정규화된다.
...
9. review 진행 출력은 기본 compact mode에서 짧은 action line으로 보인다. main/cross 단계, provider/model, 완료 시간, finding 수, gate verdict, next command만 우선 보이고, 상세 phase/lifecycle/waiting_on/next 진단은 `/progress-display stream`에서 확인한다.
