...
8. agent pass 뒤에는 Kernforge가 `/verify --full`, `/session audit`, audit ready 시 최종 semantic reviewer를 실행하고, 필요 시 `/session recover execute-safe` 또는 semantic repair pass를 실행한 뒤 다음 iteration으로 넘어간다.
...
main provider/model만 바꾸면 명시적인 analysis worker/reviewer profile은 유지된다. 이전에 따로 둔 route가 아니라 현재 main model을 다시 상속시키고 싶으면 `/model analysis clear`를 사용한다. 한쪽만 초기화하려면 `/model analysis-worker 0` 또는 `/model analysis-reviewer 0`을 사용하고, 대화형 provider 선택기에서도 `0`을 고르면 같은 동작을 한다.
...
19. 암시적 자동 model-backed review는 `review.model_review_consent`를 따른다. 기본값은 `ask`이며 `always`, `never`도 지원한다. `auto_after_change`, `auto_after_goal_iteration`, `auto_before_git_write` 같은 자동 flag는 review 대상 자격만 의미하고, 그 자체로 reviewer model 요청을 보내지 않는다. 확인 문구는 `Run model review now? [y/N/a=auto-review for this session]`이며, 비대화형 `ask`는 `skipped_no_interactive_consent`로 기록하고 건너뛴다. 명시적 `/review`, 자연어 리뷰 요청, MCP `kernforge_review`, 명시적으로 등록한 review automation은 명시 요청으로 유지된다. 자동 리뷰가 생략되거나 차단되면 reviewer verdict/skip reason과 함께 사용자가 볼 수 있었던 최초 main-model proposal/ref를 가능한 경우 노출한다.
