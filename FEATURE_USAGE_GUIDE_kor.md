...
10. analysis, investigation, simulation, performance, root-cause, fuzzing, verification, evidence, memory, checkpoint, feature, worktree, jobs, task-owner action 뒤에 출력되는 handoff block과 `/session continuity` packet을 따라가면 명령 순서를 외우지 않아도 된다.
...
15. 새 프롬프트 직전에는 `cwd`, provider/model, runtime gate, permission profile, progress display, MCP, skills, verification, memory, warning, provider route error를 compact operator footer로 보여준다. footer는 `/status`와 같은 renderer 및 compact 상태 용어를 쓰며, 좁은 터미널에서는 2줄로 나뉜다.
...
17. 직접 `!` shell 명령은 출력 전에 ok/failed 상태, 가능하면 exit code, elapsed time, 출력 줄 수, command preview를 한 줄 요약으로 보여준다. 실패한 명령은 첫 유효 error 줄과 짧은 next action도 함께 보여준다.
18. 모델이 실행한 `run_shell` 완료 요약도 첫 의미 있는 출력 줄 앞에 전체 출력 줄 수를 표시한다.
19. 직접 `!` shell output은 긴 출력일 때 compact preview를 사용한다. transcript에는 처음 80줄과 마지막 20줄을 남기고, 매우 긴 단일 라인은 문자 budget 기준으로 접은 뒤 header에 `collapsed`를 표시하며 생략된 줄 또는 문자 수를 marker로 보여준다. 한국어 locale에서는 collapse marker도 한국어로 출력된다.
...
9. permission prompt는 같은 키 계약을 쓴다. `y`는 이번만 허용, `a`는 현재 세션 동안 해당 action 허용, `n`은 거부, `Esc`는 취소다.
10. diff preview는 `y=이번만 열기`, `a=세션 편집 승인`, `n=편집 취소`, `Esc=취소`로 묻고, 자동 검증은 `y=이번만 실행`, `a=세션 자동 실행`, `n=건너뛰기`, `Esc=취소`로 묻는다.
11. `git_add`, `git_commit`, `git_push`, `git_create_pr` 같은 git 변경 도구는 별도의 `Allow git?` 세션 승인을 사용한다.
...
4. `/session events tail [n]`은 최근 session event를 JSONL record로 출력하고, `/session events export [path]`는 `.kernforge/events/<session-id>.jsonl`과 `.kernforge/events/latest.jsonl`에 durable local event stream을 저장한다.
...
현재 동작:
1. `/session continuity`는 `.kernforge/continuity/latest.md`와 `.kernforge/continuity/latest.json`을 생성한다.
...
3. 직접 실행한 `!shell` 실패는 `command_error` conversation event로 저장되므로, 이후 `/session continuity`와 최근 오류 답변이 사용자의 로그 재입력 없이 실패 맥락을 회수할 수 있다.
4. `/session jobs`는 저장된 background job/bundle 상태를 동기화해 보여주고, id 또는 `latest` 기준으로 직접 polling/cancel을 지원한다.
...
7. `/session recover`는 최신 error, verification failure, failure-repair 상태, background job, open task, next command를 `.kernforge/recovery/latest.md/json`에 더 좁은 failure runbook으로 저장한다. runbook에는 structured diagnosis, 안정적인 failure signature, action plan status, execution log가 포함된다.
8. `/session audit`는 finalizing 전에 blocker, warning, required artifact, 최신 verification, open task, background job, 최근 error, coding harness evidence를 `.kernforge/completion_audit/latest.md/json`으로 저장한다.
9. `/session recover execute-safe`는 safe-auto recovery action만 실행하고 그 상태를 기록한다. shell replay는 chaining/redirection이 없는 whitelisted verification/status 명령으로 제한된다.
10. slash action은 실행 여부만 보지 않고 기록된 artifact를 다시 확인한다. `/verify` 실패 report와 `ready=false`인 `/session audit` 결과는 recovery action 실패로 승격되고, `stop_on_failure`가 있으면 뒤 액션은 skip된다.
...
목적:
1. 사용자가 Codex식 목표를 한 번 지정하면 Kernforge가 write, diff preview, shell, git 확인으로 멈추지 않고 계속 작업하게 하되, 암시적 model-backed review는 consent policy를 따르게 한다.
...
7. goal 실행 중에는 write, diff preview, shell, git approval을 session 안에서 bypass해서 해당 확인으로 멈추지 않는다. 다만 암시적 model-backed review gate는 `review.model_review_consent`에 따라 `Run model review now? [y=이번만 실행, a=세션 자동 리뷰, n=건너뛰기, Esc=취소]`를 물을 수 있다.
...
4. `/session handoff import <path>`는 JSON 또는 markdown result를 `.kernforge/handoff/imports/*.json` 및 `*.md`로 정규화하고 conversation event를 남기며, `completed_tasks` ID가 TaskGraph node와 맞으면 completed로 표시한다.
...
9. `/session audit`는 final readiness gate를 `.kernforge/completion_audit/latest.md/json`으로 외부화해서, 사람이나 scheduler가 모델 턴 밖에서도 blocker와 warning을 볼 수 있게 한다.
...
11. worker에게 파일 앞 60줄만 주는 대신 symbol-aware evidence packet을 제공하고, claim이 `evidence_packet_ids`를 달도록 요구한다.
12. scan coverage ledger로 큰 파일, binary/non-text, unreadable, excluded skip을 run/latest artifact와 dashboard/docs에 남긴다.
13. Phase 2 structural index로 parser coverage, fallback diagnostics, symbol/reference/call-edge 후보, packet coverage metric을 run/latest artifact와 dashboard/docs에 남긴다.
14. startup, IOCTL, callback, handle/memory, RPC, asset/config, build-context, generated-artifact evidence는 directory chunking보다 graph-guided shard community를 우선 사용한다.
15. deterministic claim verifier가 매 run 실행되어 high-confidence claim이 packet, source, line, symbol, graph, security-boundary evidence 없이 fact로 승격되지 않게 한다.
16. Windows driver와 Unreal authority/config surface를 security/anti-cheat overlay로 남기고 final report, docs, dashboard에서 함께 확인할 수 있게 한다.
...
main provider/model만 바꾸면 명시적인 analysis worker/reviewer profile은 유지된다. 이전에 따로 둔 route가 아니라 현재 main model을 다시 상속시키고 싶으면 `/model analysis clear`를 사용한다. 한쪽만 초기화하려면 `/model analysis-worker 0` 또는 `/model analysis-reviewer 0`을 사용한다. analysis worker, analysis reviewer, cross-review의 대화형 provider 선택기에서도 `0`을 고르면 해당 target을 inherited/default route로 초기화한다. cross-review의 직접 초기화 명령은 `/model cross-review 0` 또는 `/model clear cross-review`다.
...
생성 문서 세트에는 run 마지막에 출력된 assistant-facing final synthesis를 그대로 보존하는 `FINAL_REPORT.md`와 architecture, security, entrypoint, build artifact, coverage ledger, structural index, evidence packet, verification, fuzz target, operation 운영 문서가 함께 들어간다.
dashboard는 이 문서들을 inline Markdown viewer로 열며, 최종 리포트나 다른 generated document가 기본 패널에서 읽기 길면 `Reader` 버튼으로 full-window reading mode를 사용할 수 있다. 상단 metric에는 skipped file 수, structural symbol 수, evidence packet 수가 함께 표시된다.
...
3. `analyze-project`가 생성하는 docs는 특정 run의 source anchor, evidence packet, coverage caveat, confidence, stale/invalidation marker를 담은 프로젝트별 운영 지식 베이스다.
...
2. `structural index`: parser coverage, fallback diagnostic, symbol anchor, reference, call-edge 후보, packet coverage metric을 함께 담는 Phase 2 인덱스
...
6. `coverage ledger`: included/skipped 파일, skip reason, skipped byte, max file size 기준
7. `evidence packets`: shard worker가 실제로 받은 source slice와 claim-level packet id
8. `vector corpus`: 임베딩 친화적인 project/subsystem/shard 문서 묶음
9. `vector ingest exports`: pgvector, sqlite, qdrant로 넘기기 쉬운 seed 파일
10. `graph_shards`, `graph_reuse`, `evidence_graph`: graph community, symbol-level reuse fingerprint, worker prompt에 들어간 evidence edge
11. `claim_verification`, `unsupported_claims`: deterministic verifier 결과, downgrade된 claim, blocking issue, follow-through command
12. `security_overlay`: Windows driver/IOCTL/callback/handle/memory/RPC/telemetry와 Unreal RPC/replication/asset/config/integrity boundary node/edge
...
2. `security` 또는 `surface` 모드의 non-Unreal Windows/driver 프로젝트도 `security_driver`, `security_ioctl`, `security_handles`, `security_memory`, `security_rpc` shard로 분리될 수 있다.
3. graph-guided planner가 `security_driver`, `security_ioctl`, `callback_registration`, `security_handles`, `security_memory`, `security_rpc`, `asset_config`, `build_context`, `generated_artifact` 같은 runtime/security community를 별도 evidence set으로 만든다.
4. incremental reuse는 file hash뿐 아니라 symbol, edge, build-context, overlay, derived graph fingerprint까지 비교한다.
3. worker와 reviewer prompt가 shard 목적에 맞는 semantic focus, evidence packet table, review checklist를 받는다.
...
2. 결과에는 typed finding, request class, lifecycle phase, route mode, request-class reason, freshness/redaction 상태, gate verdict, scope discovery, repair step, runtime gate ledger, 추천 next command가 포함된다.
3. MCP client도 `kernforge_review`를 통해 같은 구조를 받는다. 응답에는 additive `request_class`, `lifecycle` 필드와 함께 `model_plan`, `reviewer_runs`, `latest_review_freshness`, `edit_proposals`, `runtime_gate_ledger`, `single_model_second_pass`, `cross_review_triage`, `review_observability`, `scope_discovery`, `next_commands` action contract가 포함된다.
4. protocol artifact에는 action envelope, approval ledger, capability manifest, external lookup intent, artifact integrity, ledger consistency, resume sanity, state transition, route health, document gate 상태, single-model second-pass 상태, cross-review triage ledger도 포함된다. 따라서 CLI 출력, Markdown report, JSON artifact, MCP 응답이 raw model output을 MCP에 dump하지 않고 같은 lifecycle 상태를 말한다.

review UX/Ops observability:
1. 이미 구현된 runtime enforcement는 review gate ledger, cross-review triage ledger, 강제 single-model second-pass phase, pre-final answer completeness gate에 그대로 있다.
2. 새 request-class-aware orchestration은 사용자 요청을 `review_only`, `document_artifact`, `review_then_modify`, `modify_then_review`, `verification_only`, `validation_only`, `general`로 분류한다. 선택된 class, 이유, confidence, ambiguity flag, ambiguity warning, class별 final-answer contract는 `ReviewRun`, `RuntimeGateLedger`, status surface, artifact, MCP review response에 남는다.
3. single-model 모드에서 Kernforge는 이 경로를 degraded fallback이 아니라 명시적 staged lifecycle로 취급한다. compact 상태는 classify/context/review-or-implementation/second-pass/verification/final phase와 second-pass가 실행됐는지, cache됐는지, skip됐는지, 그리고 skip이 approval이 아닌 disclosure인 이유를 보여준다.
4. configured reviewer-only post-change 경로에서는 single-model policy state를 계속 보여주되, 그 reviewer가 유일한 post-change reviewer라면 primary second-pass를 중복 호출하지 않는다. reviewer output이 weak, empty, malformed, stale, timeout이면 route는 degraded로 표시하고, deterministic blocker와 필수 verification gap은 weak approval보다 강하게 유지한다.
5. cross-model 모드에서는 primary 구현 책임과 독립 reviewer 피드백을 분리한다. actionable reviewer item은 `cross_review_triage`에서 `accepted_fixed`, `accepted_deferred`, `rejected_with_reason`, `needs_user_decision` 중 하나가 되어야 하며, 수정 근거, 보류 이유, 기술적 기각 이유, 사용자 결정 guidance가 부족하면 obligation으로 남는다.
6. document-artifact 요청은 artifact-quality check를 1차 gate로 사용한다. 요청한 artifact 존재, 요청 topic coverage, placeholder/TODO-only 여부, unsupported shell/code verification claim 여부를 검사한다. document-only report 생성은 code-modification lifecycle로 승격하지 않지만, report와 source-code edit가 섞이면 modification review와 validation obligation을 다시 연다.
7. UX/Ops layer는 이 lifecycle 결정을 `/status`, `/hooks`, session dashboard, Markdown artifact, JSON artifact, MCP response로 노출한다. compact status에는 latest review id, request class, classification confidence/ambiguity, lifecycle phase, trigger/target/mode, route mode와 route quality, review/repair/document/verification/final-answer gate status, gate verdict/action, second-pass state, triage status별 count, incomplete triage blocker, 남은 obligation, final-answer correction 상태, blocker class, 다음 추천 명령이 들어간다.
8. `/status`는 기본적으로 compact하다. `/status detail`은 evidence가 많은 detail view이며 `classified_request`, `collecting_context`, `pre_write_review`, `applying_change`, `post_change_review`, `single_model_second_pass`, `cross_review_triage`, `artifact_quality_gate`, `verification`, `final_answer_contract`, `blocked`, `completed` timeline을 펼친다. 각 phase는 status, 짧은 reason, 가능한 evidence ref, blocked/skipped일 때 next safe action을 가진다.
9. `/progress-display compact`는 일반 operator 경험의 기본값이다. review progress는 `review 1/6 scope`, `review 2/6 evidence`, `main ... done`, `cross ... done`, `review decision`, `next ...` 같은 짧은 action line으로 보이고, 반복되는 전체 1->6 flow text와 raw `phase/waiting_on/next` 진단 토큰은 출력하지 않는다. `/progress-display auto`도 정상 interactive review에서는 같은 긴 flow spam을 피한다.
10. `/progress-display stream`은 상세 디버깅 모드다. stream 모드에서도 모든 progress update를 transcript에 남기되, stage/wait/next-action 진단은 raw key-value token dump가 아니라 읽을 수 있는 문장형 텍스트로 표시한다. `verbose` config alias는 계속 `stream`으로 정규화된다.
11. compact 최종 review CLI 출력은 result-first다. verdict, blocker/warning/note count, gate action, target/mode/request class, severity별 finding, report path, 추천 next command 순서로 보여주며 lifecycle/route/triage detail은 artifact와 stream/detail surface에 남긴다.
12. compact CLI에서는 같은 command를 가진 next command가 하나로 접힌다. terminal에는 command, 가장 강한 reason 또는 합쳐진 짧은 reason, confirmation 필요 여부만 보이고, Markdown/JSON/MCP/runtime ledger에는 중복을 포함한 전체 next-command record가 보존된다.
13. blocker class는 `code_repair_blocker`, `reviewer_route_problem`, `evidence_gap`, `verification_gap`, `document_artifact_quality`, `final_answer_contract`, `user_decision_required`로 분리한다. 그래서 cross-review route warning이나 document artifact detail이 primary code blocker를 가리지 않는다.
14. cross-review triage artifact는 각 finding에 id/title, reviewer route, severity, location, status, reason, required fix, fix evidence, verification evidence, safe continuation guidance를 보여준다. `needs_user_decision`은 raw review 문장에서 다음 행동을 추측하게 하지 않고 확인할 대상, 안전하게 바꿀 범위, 아직 바꾸면 안 되는 범위, 다음 명령을 제공한다.
15. 사용자가 `RF-004 수정해줘`처럼 특정 RF id를 명시하면 repair handoff는 해당 RF만 `RepairFindings`와 pre-fix repair obligation으로 carry한다. 나열되지 않은 RF를 의도적으로 고치거나 해결했다고 보고하지 말아야 하며, 선택된 RF가 다른 RF와 분리 불가능하면 범위를 넓히기 전에 결합 사유를 보고하고 사용자 결정을 기다린다.
16. single-model mode는 degraded fallback이 아니다. status/report는 `single_model_second_pass_ran`, `single_model_second_pass_cached`, `single_model_second_pass_skipped`, `cross_model_review_ran`, `reviewer_only_post_change_review_used`를 구분하며 single-model second pass를 독립 cross-review처럼 설명하지 않는다.
17. document-artifact flow는 artifact path, artifact-quality status, 필요한 경우 source-review evidence status, verification skip 또는 required reason, 남은 limitation을 보여준다. document artifact final-answer correction은 code repair가 아니라 final-answer contract correction으로 표시한다.
18. final-answer completeness correction은 changed-file disclosure, review/self-review disclosure, validation disclosure, remaining-risk disclosure, review-only findings-first/no-edit formatting, document-artifact path/quality/verification/limitation disclosure 같은 lifecycle fact로 노출한다. 내부 prompt text는 노출하지 않고, "완료", "패치됨", "구현됨" 같은 generic legacy 완료 문구만으로는 evidence를 충족하지 않는다.
19. MCP operator-card field는 additive/backward-compatible이다. `lifecycle_timeline`, `compact_status`, `blocker_summary`, `route_quality`, `final_answer_contract_status`, `next_recommended_command`를 제공하므로 MCP consumer는 prose나 raw model output을 파싱하지 않고 operator card를 그릴 수 있다.
20. 알려진 한계: raw model output은 local artifact ref 뒤에 남기고 MCP response에 dump하지 않는다. MCP field는 additive/backward-compatible로 유지한다. generated-document artifact skip behavior는 code review gate와 계속 분리한다. ambiguity가 높은 자연어 분류는 보수적으로 남긴다. 전체 패키지 테스트는 한 번에 빠르게 끝난다고 가정하지 말고 cluster 단위 진단이 필요할 수 있다.
...
3. UX/Ops 회귀는 `go test ./cmd/kernforge -run "TestOperator|TestRuntimeGate|TestReviewMCPResponse|TestMCPStatus|TestCrossReview|TestSingleModel|TestPreFinalHarness|TestFinalAnswerContract" -count=1`처럼 묶는다.
...
3. cross-review provider를 대화형으로 고를 때 `0`을 선택하면 독립 route를 지우고 기본 single-model mode로 되돌린다. 직접 명령은 `/model cross-review 0`이다.
4. gate와 사용자 흐름이 허용할 때만 실행으로 이어진다.
...
5. `@file:line-line 리뷰해줘` 같은 자연어 리뷰 요청은 `/review selection`으로 라우팅하고, focused review-and-fix 요청은 `review_then_modify`로 분류해 먼저 리뷰를 실행한 뒤 최신 finding을 기준으로 repair 흐름을 이어간다.
...
9. review 진행 출력은 기본 compact mode에서 짧은 action line으로 보인다. main/cross 단계, provider/model, 완료 시간, finding 수, review decision, next command만 우선 보이고, 상세 stage/wait/next-action 진단은 `/progress-display stream`에서 읽을 수 있는 텍스트로 확인한다.
...
14. 수정 전 리뷰 finding은 edit tool 실행 전에 사용자에게 먼저 노출된다. repair turn에 구조화된 RF 항목이 있으면 Kernforge가 implementation model 시작 전에 해당 ID와 조치 방향을 담은 deterministic `검토 결과:` 요약을 먼저 출력하고 세션에 저장한다. 사용자가 `RF-004 수정해줘`처럼 특정 RF id를 지정하면 그 RF만 repair guidance, `RepairFindings`, pre-fix obligation에 남기고 다른 RF는 해결했다고 보고하지 않는다. 이후에도 visible summary가 없으면 edit tool 실행 전 가드가 다시 막는다.
...
18. Claude Code CLI 기본 선택지는 현재 Claude family version을 표시하지만, 실제 CLI 실행에는 `sonnet`, `opus`, `haiku` 같은 안전한 alias를 넘긴다. `/review`, 자연어 리뷰, 수정 전 repair check는 main-first로 동작한다. active main model이 로컬 evidence로 첫 구조화 리뷰를 만들고, optional cross route가 설정돼 있으면 같은 evidence와 primary draft를 받아 cross-check로 다시 본다. distinct cross route가 없으면 prompt 지시만 믿지 않고 primary route로 별도 `single_model_second_pass` review phase를 실행한다. 이 2차 pass는 원 요청, touched files, relevant diff, implementation reply, 최신 verification summary, 1차 review를 받고 기존 `ReviewFinding`과 같은 구조로 결과를 반환하므로 동일 gate/repair loop가 소비한다. 승인된 second-pass fingerprint는 cache에 저장되어 같은 diff를 무한 반복 검토하지 않는다. 선택된 request class는 이 evidence가 read-only review인지, review-before-repair evidence인지, post-change self-review evidence인지, document-artifact validation disclosure인지 결정한다. domain 전문성은 `security`, `design`, `false_positive`, `regression`, `test`, `final_gate` lens로 prompt에 주입된다. cross reviewer가 실패하거나 빈 응답을 반환하거나 `weak` 품질로 끝나도 run은 degraded 상태로 표시될 뿐, main review finding 보고나 repair loop 시작 자체를 막지는 않는다. actionable cross reviewer finding은 `cross_review_triage`에 저장되고 `accepted_fixed`, `accepted_deferred`, `rejected_with_reason`, `needs_user_decision` 중 하나로 결정되어야 한다. 수정 근거, 보류 이유, 기술적 기각 이유, 사용자 결정 guidance가 부족하면 deterministic review blocker가 gate를 막는다.
19. 암시적 자동 model-backed review는 `review.model_review_consent`를 따른다. 기본값은 `ask`이며 `always`, `never`도 지원한다. user config(`~/.kernforge/config.json`) 또는 trusted workspace config(`.kernforge/config.json`)의 `"review": {"model_review_consent": "ask"}` 값으로 `ask`, `always`, `never` 중 하나를 설정한 뒤 `/reload`하면 적용된다. `/status`, `/config`, `/model cross-review status`에서 effective 값을 확인할 수 있다. `auto_after_change`, `auto_after_goal_iteration`, `auto_before_git_write` 같은 자동 flag는 review 대상 자격만 의미하고, 그 자체로 reviewer model 요청을 보내지 않는다. 확인 문구는 `Run model review now? [y=이번만 실행, a=세션 자동 리뷰, n=건너뛰기, Esc=취소]`이며, `y`는 1회 실행, `n`은 `skipped_by_user`, `a`는 현재 세션의 암시적 model review 자동 허용, 비대화형 `ask`는 `skipped_no_interactive_consent`로 기록한다. 명시적 `/review`, 자연어 리뷰 요청, MCP `kernforge_review`, 명시적으로 등록한 review automation은 명시 요청으로 유지된다. 자동 리뷰가 생략되거나 차단되면 reviewer verdict/skip reason과 함께 사용자가 볼 수 있었던 최초 main-model proposal/ref를 가능한 경우 노출한다.
20. cross review route는 기본적으로 최소 `effort=high`를 사용하고, 저장된 `low`/`medium` 값도 reviewer 요청을 만들 때 `high`로 올린다. focused pre-fix bug-hunt review도 이 최소값을 유지한다. pre-write review는 여전히 hard edit gate다. 실제 edit preview가 생긴 뒤에는 필수 main/cross reviewer가 실패하거나 빈 응답을 반환하거나 `weak` 품질이면 `insufficient_evidence`로 write를 막고, Kernforge는 파일을 건드리기 전에 reviewer route 문제를 보고한다. 이때도 implementation model에게 재시도나 웹 근거 수집을 시키지 않는다. 단, 메인 모델의 pre-write 1차 리뷰가 usable이면 중단 응답에 `메인 모델 리뷰 기준으로 진행` 선택지를 함께 보여준다. 사용자가 그 문구로 명시 승인하고 interactive diff preview가 가능한 경우에만 다음 pre-write review에서 cross reviewer 실패를 degraded evidence로 기록하되 hard blocker로는 보지 않고, 그래도 실제 쓰기 전에는 기존 diff preview 확인을 반드시 거친다.
21. pre-write review가 diff preview로 진행할 수 있다고 판단하면, diff preview 질문 전에 최종 검토 결과 본문을 사용자에게 먼저 출력한다. 이 본문은 판정, blocker/warning 수, 수정 확인 대상, 남은 검토 항목, evidence, impact, required fix, test recommendation을 포함하며 긴 필드도 `...`로 잘라 조치 기준을 숨기지 않는다.
22. build/test verification gap은 edit preview를 막는 semantic blocker가 아니라 post-edit obligation으로 처리한다. verification report는 현재 patch transaction 이후에 생성됐고 changed path를 덮을 때만 current로 보며, 오래된 session verification이나 persisted verification history는 review blocker가 아니라 runtime gate warning과 `/verify --full` next action으로 내려간다. artifact를 만들 수 있는 build/test shell 명령은 diff preview와 같은 pinned confirmation 형식인 `자동 검증을 실행할까요? [y=이번만 실행, a=세션 자동 실행, n=건너뛰기, Esc=취소]`으로 묻고, `-prompt -y` 같은 비대화형 bypass 실행에서는 diff preview처럼 자동 승인한다. progress line도 승인 전에는 `검증 승인 확인 중`으로 표시해서 실제 실행/거절/증거 상태를 분리한다. 이 lifecycle은 Codex의 command approval처럼 승인 요청, 사용자 결정, 실제 실행 결과와 evidence 기록을 분리한다. background verification은 시작 시 `pending` evidence gap이며, 완료 결과가 확인될 때만 pass/fail evidence가 된다. 검증이 거절되거나 스킵되면 이후 같은 턴의 shell 검증 재시도와 `latest` background poll은 새 shell/progress status를 내기 전에 `NOT_EXECUTED`로 막는다. 최종 답변이 skipped verification을 고지하는 것은 disclosure 의무를 충족할 뿐 성공 evidence를 만들지 않으므로, 변경 path에 성공 검증 report가 없으면 edit-loop status는 `risk_accepted`로 남긴다. background job bundle metadata는 job-list evidence를 `job_entries`에 저장하고 scalar `job_status`는 단일 job 상태에만 사용한다. 편집 후 자동 검증이 실패하면 Kernforge는 모델에게 수정을 더 시키기 전에 실패 증거가 현재 patch scope에 속하는지 판정한다. 명령/scope 또는 실패 라인이 변경 path를 직접 가리키는 경우만 좁은 repair loop로 이어가고, workspace 또는 sibling-file 실패가 변경 path와 연결되지 않으면 검증 risk로 보고하되 unrelated source/project file 수정으로 확장하지 않는다. out-of-scope 자동 검증 실패 이후 같은 턴에서 build/test/verification 재시도나 probing이 나오면 이것도 `NOT_EXECUTED`로 막아 모델이 대체 빌드 수리로 빠지지 않고 외부/환경성 blocker를 보고하게 한다. 검증 명령이 만드는 build artifact 변경은 허용하지만 source/config file 변경은 edit review gate 밖에서 계속 차단한다. C++/MSBuild adaptive verification은 변경 source를 포함하는 가장 가까운 `.vcxproj`를 먼저 사용하고, 해당 project가 선언한 `Configuration|Platform` 속성을 같이 넘겨 MSBuild가 지원하지 않는 기본 platform으로 빠지지 않게 한다. 솔루션 전체 빌드는 명시적 full verification에 남긴다. configured/detected verification tool path는 shell 경계에서 정규화하며, PowerShell에서는 quoted executable 앞에 `&`를 붙여 detected MSBuild/CMake/CTest/Ninja 경로가 문자열 literal로 파싱되지 않게 한다. verification summary는 논리 명령과 실제 resolved shell command가 다르면 둘 다 보존하고, missing-tool 판정은 빌드 출력의 임의 문구가 아니라 primary executable 기준으로만 한다.
...
22. 리뷰된 repair가 다시 pre-write gate를 통과하지 못하거나 좁은 확인 예산을 소진하면, Kernforge는 리뷰 미통과를 먼저 알리고 최신 리뷰 결과와 마지막 수정안을 보여준 뒤 `계속 수정할까요? [y=계속, n=중지]`로 묻는다. 이 confirmation은 자연어 prompt가 아니라 session state다. `y`만 저장된 review/proposal 기준으로 이어가고, `n`은 멈춘다.
...
26. 현재 review hardening 계층은 review action envelope, approval ledger, capability manifest, external lookup intent, artifact integrity, ledger consistency, resume sanity, route health를 review run에 기록한다. replay fixture는 reviewer route failure, omission/truncation, patch mismatch loop, local web block, pre-fix repair obligation, compact final review output, RF-scoped repair handoff, MCP response contract를 고정한다.
27. 수정 또는 로컬 리뷰 final answer가 사용자에게 보이기 전에 pre-final coding harness가 request class별 완료 사실을 검사한다. 수정 흐름은 변경 파일 또는 무변경 명시, review/self-review 결과, validation 결과 또는 미실행 이유, remaining risk 또는 known blocker 없음이 필요하다. review-only 흐름은 read-only 상태를 유지하고 findings-first로 시작해야 하며, finding이 없으면 남은 test/evidence risk를 말해야 한다. review-then-modify 흐름은 review finding 근거를 먼저 보여준 뒤 scoped patch, post-change review 또는 second-pass state, validation, residual risk를 요약해야 한다. generated document artifact와 단순 status/command 턴은 code-review completion gate에서 skip해 과차단하지 않지만, document artifact path와 artifact-quality validation disclosure는 필요하다.
...
현재 연동:
1. `/new-feature <task>`는 `feature.json`, `spec.md`, `plan.md`, `tasks.md`를 만든다.
2. 생성된 feature는 세션의 active feature로 기록된다.
3. `/new-feature`는 active feature 상태, artifact 경로, task preview, 다음 action을 보여준다.
4. `/new-feature next`는 저장된 plan 실행, verification 안내, 통과 후 close를 현재 상태에 맞게 처리한다.
...
2. feature 생성 시 `spec.md`, `plan.md`, `tasks.md`가 생성되고, blocked 상태에서 `next`를 이어가면 다시 생성된다.
3. `/new-feature next`는 저장된 plan을 실행하고 `implementation.md`를 남긴다.
4. 구현 뒤에는 `/new-feature next`가 먼저 verification을 안내하고, 통과 기록이 있을 때만 close로 진행한다.
