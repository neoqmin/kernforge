...
- Tools: `kernforge`, `kernforge_review`, `kernforge_fuzz`, `kernforge_guide`, `kernforge_look`, `kernforge_status`, `kernforge_latest_analysis`, `kernforge_read_analysis_doc`, `kernforge_evidence_search`, `kernforge_memory_search`, `kernforge_verification_history`, `kernforge_analysis_context`, `kernforge_artifact_index`, `kernforge_fuzz_targets`, `kernforge_source_scan`, `kernforge_source_candidate_list`, `kernforge_source_candidate_show`, `kernforge_fuzz_workflow`, `kernforge_fuzz_func`, `kernforge_fuzz_func_preview`, `kernforge_fuzz_func_build`, `kernforge_fuzz_func_status`, `kernforge_fuzz_artifacts`, `kernforge_fuzz_campaign_status`, `kernforge_fuzz_campaign_run`, `kernforge_verify`, `kernforge_analyze_project`, `kernforge_find_root_cause`
...
Analysis and root-cause tools require a configured provider/model in KernForge config or the resumed session. Status, latest-analysis reads, focused analysis context from saved artifacts, evidence/memory/history search, artifact index, guided routing, fuzz target reads, source-level fuzz planning, function fuzz preview/build-only, campaign status, and verification planning can be used without model access. `kernforge_review` can run read-only deterministic/no-model checks, but model-backed review findings require provider/model access.
...
When the user explicitly asks KernForge to review code, a plan, a PR, a selected region, a final answer, or an analysis target, call `kernforge_review`. It uses the same common review harness as CLI `/review`, writes structured `.kernforge/reviews/latest.json` and `.md` artifacts, returns `machine_status`, gate details, model-role status, findings, waivers, `scope_discovery`, freshness/redaction state, `latest_review_freshness`, `edit_proposals`, `runtime_gate_ledger`, and action-oriented `next_commands` with reason, timing, safety, and expected-result metadata, and replaces the older review-code-only MCP surface. Provider behavior controls review token caps, omission retry budgets, schema strictness, and recovery prompts; weak or incomplete high-severity findings are downgraded to evidence-gap warnings unless they carry enough concrete evidence to block the gate. Treat stale `latest_review_freshness` or blocked `runtime_gate_ledger` as a stop sign for final answers and write-side MCP actions until the displayed next command is handled.
