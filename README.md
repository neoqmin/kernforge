...
- If a pre-write repair attempt still does not pass review, Kernforge now stops before another broad loop, shows the latest review result and edit proposal, records a pending repair confirmation in the session, and asks `Should I keep repairing from this review result? [y/N]`. Only `y` resumes from the stored findings/proposal; `n` stops; natural-language answers are rejected so the next action is explicit.
