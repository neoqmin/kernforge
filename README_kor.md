...
- review progress는 기본 `compact` mode에서 반복적인 전체 1->6 flow 설명을 숨기고 `review 1/6 scope`, `main ... done`, `cross ... done`, `gate ...`, `next ...` 같은 action line만 보여준다. `/progress-display stream`을 선택하면 phase, lifecycle, waiting target, next transition 같은 상세 진단을 transcript에 남긴다.
...
- `progress_display`가 진행 표시 방식을 제어하며 기본값은 일반 review/coding 작업을 읽기 쉽게 유지하는 `compact`이다. REPL에서는 `/progress-display auto|compact|stream`으로 바로 바꾼다. `compact`는 짧은 operator line과 footer 중심으로 조용하게 보여주며, `auto`는 반복적인 상세 review flow text를 피하면서 중요한 durable event만 남기고, `stream`은 상세 디버깅을 위해 모든 progress update를 본문에 지속 기록한다.
...
| `progress_display` | runtime 진행 표시 방식. 기본값 `compact`는 일반 진행 상태를 transient/footer 중심으로 유지하고 review 결과를 action 중심으로 보여준다. `auto`는 반복적인 상세 review flow text를 피하면서 중요한 tool/model/project-analysis event만 남기며, `stream`은 긴 작업 디버깅을 위해 모든 progress update를 transcript에 기록한다. |
...
- `/status`는 현재 세션과 런타임 상태를 보여준다. 예를 들어 approval 상태, 세션 id, 메모리/검증/MCP 카운트와 runtime gate ledger가 여기에 들어간다. 이 gate 섹션은 최신 review가 fresh인지, gate가 왜 통과/차단/degraded/needs action인지, 어떤 blocker class가 활성인지, 다음 복구 명령이 무엇인지 보여준다.
...
- `/progress-display`는 runtime 진행 표시 모드를 보여주거나 저장한다. 기본값은 `compact`이며 Codex CLI처럼 조용한 operator view를 우선한다. `auto`는 반복적인 상세 review flow text 없이 중요한 tool/model ledger만 남기고, `stream`은 모든 progress update를 transcript에 남긴다.
