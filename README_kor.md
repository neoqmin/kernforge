...
- `ollama`, `anthropic`, `openai`, `openrouter`, `openai-compatible`, `opencode`, `opencode-go`, `codex-cli` provider 지원
...
- 설정된 OpenCode API key로 모델 목록을 조회하므로 picker가 실제 계정/구독에서 사용 가능한 모델을 기준으로 표시된다.
...
- OpenCode Go key로 모델 목록을 조회하고 Go 구독 라우팅을 Zen pay-as-you-go 라우팅과 분리한다.
