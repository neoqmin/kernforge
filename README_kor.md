...
Kernforge는 `project analysis -> review -> fuzzing/root-cause investigation -> adaptive verification -> evidence store -> persistent memory -> hook policy -> checkpoint/rollback` 흐름을 중심으로, driver, telemetry, memory-scan, Unreal 보안 작업을 더 안전하고 일관되게 진행할 수 있도록 설계되어 있습니다.

## Claude Code 및 Codex와의 비교

| 비교 축 | Kernforge | Claude Code | Codex |
|---|---|---|---|
| 가장 잘 맞는 용도 | Windows security, anti-cheat, telemetry, driver 워크플로우, 대형 프로젝트 분석, evidence 기반 검증 | 범용 agentic coding, configurable hooks, subagents, 외부 연동, 팀 정책 workflow | 범용 코딩 에이전트 작업, 로컬 편집 루프, task delegation, automation, PR 중심 workflow |
| 핵심 강점 | 큰 워크스페이스를 재사용 가능한 project intelligence, security docs, fuzz target, verification history, evidence, persistent memory로 바꾼다 | hooks, subagents, MCP식 연동, 조직별 workflow 구성 능력이 강하다 | 작업을 맡기면 inspect, edit, test, recover, summarize를 자연스럽게 끝까지 굴린다 |
| 대화 기억 | conversation event, active state, recent error, recovery brief, suggestion memory, task graph, session dashboard, continuity packet, persistent memory를 저장한다 | project instruction과 agent 설정을 통한 대화 context 유지가 강하다 | thread/workspace awareness와 task continuity가 매우 강하다 |
| 선제 판단 | `SituationSnapshot` 기반 rule/data-driven 판단으로 verification, stale docs, fuzz gap, provider failure, checkpoint/worktree, PR review, automation follow-up을 제안한다 | hook, subagent, project convention으로 workflow가 잘 정의됐을 때 강하다 | 구현 중 다음 실용 행동을 고르는 능력이 강하다 |
| 검증과 증거 | adaptive verification, verification history, evidence store, dashboard, memory promotion, fuzz result gate가 1급 기능이다 | tool loop는 강하지만 evidence modeling은 사용자/프로젝트 구성에 의존한다 | test/command loop는 강하지만 domain evidence modeling은 범용적이다 |
...
| 자동화 성숙도 | 로컬 MVP: `/automation`, interval schedule due 판단, automation digest/monitor/watch, process-detached daemon, notify artifact와 webhook transport, recurring verification slot, `/session jobs`, `/session recover`, `/session continuity`, `/session audit`, issue label/assignee/milestone을 포함한 `/review pr --github --draft-comments|--post-comments|--resolve-thread|--create-issue` report, `/session handoff`, `/session dashboard --html`, suggestion-to-task graph 구현. cloud job은 남았다 | hook과 외부 workflow 연동을 통한 자동화 구성이 강하다 | automation과 PR/task workflow 방향이 성숙하다 |
| 트레이드오프 | 더 전문적이고 evidence-heavy하지만, 범용 생태계와 desktop/cloud polish는 아직 작다 | 설정/확장 생태계는 강하지만, Windows security/fuzz workbench 깊이는 기본 내장되어 있지 않다 | 범용 agent 경험은 더 매끄럽지만, 보안/퍼징 전문 지식은 기본 내장 범위가 아니다 |
...
Kernforge의 source-level fuzzing은 코드를 먼저 컴파일하지 않아도 함수 입력 모델, 문제가 되는 코드 위치, 트리거 값, 하네스와 리포트 산출물 경로를 한 번에 정리한다. Review finding도 `path: file:line` 또는 `line: n`으로 들어온 위치 anchor를 구조화해 보존하므로, 수리 handoff와 리뷰 리포트가 prose 안의 줄 번호에 덜 의존한다. Codex 앱에서 MCP로 호출하면 아래처럼 `Result`, `Top candidate`, `Problem location`, `Trigger conditions KernForge generated`, `Artifacts`가 먼저 보이도록 설계되어 있다.
...
- 재사용 가능한 knowledge pack, symbol-aware evidence packet, scan coverage ledger, performance lens, 운영 문서, HTML 대시보드를 만드는 multi-agent project analysis
- 증상 기반 `/find-root-cause`와 내장 `/root-cause-patterns` 지식 pack
- final answer 직전 acceptance, artifact quality, scenario replay, subagent evidence, test impact, open task, verification, background job 상태를 점검하는 coding harness와 `/session audit` readiness artifact
- 최신 review transaction, patch transaction, verification report, completion audit, final-answer review를 하나로 묶는 runtime gate ledger. `/status`, `/hooks`, session dashboard, review artifact, MCP 응답, final-answer prompt에서 request class, classification confidence/ambiguity, lifecycle phase, single-model/cross-model route mode, route quality와 degraded reason, review/repair/document/verification gate status, second-pass state, cross-review triage count, blocker, waiver, obligation, next command를 같은 의미로 보여준다.
- `TaskState`, `TaskGraph`, node-aware recovery, executor guidance를 갖춘 구조화된 interactive orchestration
- built-in task ownership profile catalog와 editable/read-only ownership routing, 선택적 모델 override
- node별 editable ownership/lease, task-owner worktree lease, session-level worktree isolation
- 대화형 REPL, `-prompt` 기반 one-shot 실행, scheduler용 `-command` slash command 실행
- `/goal`, `-goal`, `-goal-file` 기반 Codex식 autonomous goal. prompt 또는 markdown 목표를 먼저 기록할 수 있고, 명시 실행 시 구현, 자체 리뷰, adaptive verification, completion audit, 최종 semantic review, recovery를 반복한다. goal 실행 중 write, diff preview, shell, git 승인은 세션 안에서 bypass하지만, 암시적 model-backed review gate는 여전히 `review.model_review_consent`를 따른다.
- `openai-codex-subscription`, `openai-codex-cli`, `openai-api`, `anthropic-claude-cli`, `anthropic-api`, `DeepSeek`, `openrouter`, `opencode`, `opencode-go`, `ollama`, `lmstudio`, `vllm`, `llama.cpp`와 명시적 OpenAI-compatible route 지원
...
- 파일, structured edit proposal, patch fallback, 셸, git 중심 도구 호출
...
- `/session continuity`, `/session recover`, `/session recover execute-safe`, `/session audit`, `/session jobs`, local event export, session dashboard를 통한 natural failure recovery
...
- adaptive verification, 검증 이력 대시보드, checkpoint, rollback. artifact를 만들 수 있는 build/test 명령은 Codex식 승인 lifecycle을 사용해 실행 전에는 검증 승인 요청으로 표시하고, 사용자가 거절하면 skipped/declined 상태로 기록하되 성공 evidence로 계산하지 않는다. declined 또는 prompt 실패 자동 검증은 skipped verification report로 영속화하고 pending verification을 유지하며, 완료된 검증처럼 표시하지 않는다. 단, generated document만 만든 턴은 예외다. 결정적 artifact-quality check가 문서를 승인했고 최종 답변이 근거 없는 검증 성공을 주장하지 않으면, Kernforge는 shell validation이나 review loop로 재진입하지 않고 self-driving task state를 완료 처리한다. 같은 턴에서 이어지는 검증 재실행이나 background poll 시도는 사용자가 명시적으로 검증 실행을 승인하기 전까지 새 progress line을 내기 전에 `NOT_EXECUTED`로 접는다. skipped automatic verification도 같은 턴의 모델 주도 검증 재시도를 차단하며, 변경 파일에 successful verification evidence가 없으면 최종 답변이 "검증을 실행하지 않았다"고 올바르게 고지해도 edit-loop ledger는 `risk_accepted`로 남긴다. 편집 후 자동 검증 실패가 현재 patch scope 밖으로 판정된 경우에도 같은 턴의 build/test/verification 재시도는 `NOT_EXECUTED`로 접어서, 모델이 unrelated project failure를 더 탐색하지 않고 외부/환경성 blocker로 보고하게 한다. 비대화형 `-prompt -y` 실행은 diff preview와 같은 기준으로 검증 prompt를 자동 승인해 실제 검증을 시작하고, bypass가 아닌 비대화형 실행은 승인을 추측하지 않고 skipped로 남긴다. 비대화형 단발 실행(`-prompt`, `-command`, `-goal`, `-goal-file`)은 ambient keyboard watcher를 붙이지 않아 장시간 model/review/goal loop가 외부 콘솔 입력을 사용자 취소로 오인하지 않는다. generated analysis, fuzz, manifest evidence는 실행 command가 없는 informational verification output으로만 보관해, 신뢰할 수 없는 evidence text가 shell input으로 승격되지 않게 한다. background job metadata는 scalar `job_status`와 list형 `job_entries`를 분리해 상태값과 log evidence가 한 key에 섞이지 않게 한다.
- out-of-scope 자동 검증 실패가 확정되면 다음 모델 턴은 tool definition을 빼고 final-answer-only로 전환한다. local/scripted route가 그래도 tool call을 내면 Kernforge는 실제 실행 없이 `NOT_EXECUTED`와 최종 답변 전용 guidance만 돌려줘 read/probe/build 루프로 흐르지 않게 한다.
- hook engine, workspace hook rules, evidence-aware push/PR policy
- plan, code, selection, PR, goal, final, analysis, 수정 전, 쓰기 전, 변경 후, MCP review를 같은 기반에서 처리하는 공통 `/review` 하네스. main-first 리뷰, 선택적 cross reviewer, request-class-aware orchestration, typed action envelope, 분리된 approval ledger, route health 기반 retry 억제, 로컬 코드 수리 중 웹 검색 차단, pre-write repair 실패 후 명시적 `y=계속, n=중지` continuation, artifact integrity 검사, replay fixture, diff preview 전 최종 리뷰 본문 출력을 포함한다. 자연어 리뷰/수정/goal 실행에는 Codex-grade 요청 처리 계약을 주입하고, 이제 핵심 계약은 runtime에서 강제된다. 요청은 `review_only`, `document_artifact`, `review_then_modify`, `modify_then_review`, `verification_only`, `validation_only`, `general`로 confidence, ambiguity warning, class-specific contract와 함께 분류되고, 그 이유가 `ReviewRun`, runtime gate, status/artifact, MCP 응답에 남는다. distinct cross route가 없으면 별도 single-model second-pass review call을 실행하거나 skipped reason을 명시하고, reviewer-only post-change 경로에서는 single-model policy state를 유지하되 중복 second-pass 호출을 피한다. actionable cross-review finding은 `cross_review_triage`에서 `accepted_fixed`, `accepted_deferred`, `rejected_with_reason`, `needs_user_decision` 중 하나로 조정해야 한다. `needs_user_decision`은 확인할 대상, 안전하게 바꿀 범위, 아직 바꾸면 안 되는 범위, 다음 명령을 포함한다. 문서 artifact는 artifact 존재, 요청 topic coverage, placeholder/TODO-only 여부, unsupported verification claim 여부를 기준으로 판단해 code review나 shell verification으로 과차단하지 않는다. 수정/리뷰/문서 final answer가 변경 파일, review 결과, validation 결과, artifact 품질, limitation, remaining risk 같은 class-specific fact를 빠뜨리면 pre-final coding harness가 내부 수정 턴으로 막고, generic legacy 완료 문구만으로는 통과시키지 않는다. UI-polish review도 changed path가 style/asset/markup-only임을 증명하지 못하면 primary reviewer coverage를 함께 요구하며, 실행 가능한 source 파일은 이름이 UI처럼 보여도 primary correctness coverage를 유지한다.
- 암시적 model-backed review 요청은 `review.model_review_consent`를 따른다. 기본값은 `ask`이고 `always`, `never`를 지원한다. user config(`~/.kernforge/config.json`) 또는 trusted workspace config(`.kernforge/config.json`)의 `"review": {"model_review_consent": "ask"}` 값으로 `ask`, `always`, `never` 중 하나를 설정한 뒤 `/reload`하면 적용된다. `/status`, `/config`, `/model cross-review status`에서 effective 값을 확인할 수 있다. 기존 automatic review flag는 리뷰 대상 자격만 의미한다. 확인 문구는 `Run model review now? [y=이번만 실행, a=세션 자동 리뷰, n=건너뛰기, Esc=취소]`이며, `y`는 1회 실행, `n`은 `skipped_by_user`, `a`는 현재 세션의 암시적 review 자동 허용, 비대화형 `ask`는 `skipped_no_interactive_consent`로 기록한다. 명시적 `/review`, 자연어 리뷰 요청, `kernforge_review`, 명시적으로 등록한 review automation은 추가 model-review consent prompt 없이 명시 요청으로 처리된다. 자동 리뷰가 생략되거나 reviewer가 차단하더라도 사용자가 볼 수 있었던 최초 main-model proposal/ref는 가능한 경우 review/session artifact에 보존되고 출력된다.
...
- worker prompt는 이제 파일 앞부분만 받는 대신 symbol-aware evidence packet을 받는다. claim은 `evidence_packet_ids`를 함께 제시해야 하며, packet 근거가 없는 high-confidence claim은 사실로 승격하지 않고 중간/낮은 신뢰도 또는 unknown으로 낮춘다.
- 각 실행은 `coverage_ledger.json`과 `evidence_packets.json`을 만들고 `latest/`에도 mirror한다. 큰 파일, binary/non-text, unreadable skip과 worker가 실제로 본 source slice를 나중에 감사할 수 있다.
- 각 실행은 Phase 2 `structural_index.json`도 만든다. 이 파일에는 parser coverage, fallback diagnostics, symbol/reference/call-edge 후보, packet coverage metric이 들어가며 Tree-sitter는 cgo/tag opt-in adapter이고 기본 Windows 빌드는 heuristic fallback parser를 유지한다.
- dedicated reviewer profile이 없는 single-model 구성에서는 reviewer를 건너뛴 shard를 `approved`로 세지 않고 `model_review_skipped`로 분리한다.
...
- knowledge pack 외에도 parser-backed `structural_index.json`, `structural_index_v2`, Unreal semantic graph, vector corpus, vector ingestion export를 함께 생성
...
- generated docs와 `dashboard.html`을 함께 생성해 최신 프로젝트 지식 베이스를 모듈/기능 구조 맵과 다크 테마 정적 document portal로 확인한다. portal은 최종 리포트와 generated docs 본문 검색, source anchor, coverage ledger, structural index health, evidence packet index, graph-linked stale section diff, trust-boundary/attack-flow view, evidence/memory drilldown, docs-backed vector corpus reuse를 제공한다.
...
- `/verify`는 마지막에 `Verification handoff`를 출력한다. 실패 시 repair/retry dashboard로, 성공 시 checkpoint와 active tracked feature의 다음 lifecycle action으로 안내하며 native fuzz finding도 targeted planner step으로 끌어온다.
- `/session recover execute-safe`는 `/verify`가 실패 report를 남기거나 `/session audit`가 `ready=false` artifact를 남기면 slash command가 실행됐다는 이유만으로 완료 처리하지 않고 recovery action을 실패로 기록한다.
...
- evidence와 memory 화면은 조치가 필요한 record를 기준으로 `/verify`, source dashboard, `/memory confirm`, `/memory promote`, dashboard review로 되돌아가는 handoff를 출력한다.
...
- `/verify tools detect`, `/verify tools set|clear <tool>` 기반 Windows verification tool 경로 탐지와 override
...
- 편집 후 자동 verification. build/test 검증 확인도 diff preview와 같은 pinned `y=이번만, a=세션, n=건너뛰기, Esc=취소` 승인 계약을 사용한다.
...
- progress line은 사람이 읽는 상태 문장으로 출력된다. 긴 대기 중에는 primary model, cross reviewer, local tool, verification command, final-answer correction 중 무엇을 기다리는지 표시하되 raw `phase=... waiting_on=...` 토큰을 일반 화면에 그대로 내보내지 않는다.
- review progress는 기본 `compact` mode에서 반복적인 전체 1->6 flow 설명을 숨기고 `review 1/6 scope`, `main ... done`, `cross ... done`, `review decision`, `next ...` 같은 action line만 보여준다. `/progress-display stream`을 선택해도 stage/wait/next-action 진단은 key-value dump가 아니라 읽을 수 있는 텍스트로 남긴다.
- compact 최종 review 출력은 result-first다. verdict와 blocker/warning/note count를 먼저 보여주고, finding을 severity별로 묶은 뒤 report path와 하나의 next recommended command를 출력한다. 같은 command가 여러 reason으로 생성되면 terminal에서는 한 번만 보이고, Markdown/JSON/MCP/runtime ledger에는 전체 next-command record가 남는다.
...
- `RF-004 수정해줘`처럼 사용자가 특정 RF id를 지정하면 repair handoff는 그 항목만 `RepairFindings`와 pre-fix obligation으로 carry한다. 나열되지 않은 RF를 의도적으로 고치거나 해결했다고 보고하지 않으며, 선택 RF가 다른 RF와 분리 불가능하면 범위 확장 전에 사용자 결정을 요구한다.
- 로컬 코드 수리는 사용자가 외부 리서치를 명시적으로 요청하지 않는 한 web/search/browser MCP tool로 review gate를 만족시키려 하지 않는다.
- final answer 직전 coding harness와 `/session audit`가 acceptance contract, 실제 변경 path, artifact 생성 여부, artifact 내용 품질, scenario replay 상태, subagent/reviewer 근거, test impact, open task, verification, background job 상태와 필수 completion fact를 검토한다.
...
- permission prompt는 같은 키 계약을 쓴다. `y`는 이번만 허용, `a`는 현재 세션 동안 해당 action 허용, `n`은 거부, `Esc`는 취소다.
- `Open diff preview?`는 `y=이번만 열기`, `a=세션 편집 승인`, `n=편집 취소`, `Esc=취소`로 묻는다. 자동 검증은 `y=이번만 실행`, `a=세션 자동 실행`, `n=건너뛰기`, `Esc=취소`로 묻는다. 미리 설정된 one-shot preview approval은 Kernforge가 다시 묻기 전에 먼저 소비하므로 scripted 또는 recovery preview 결정이 두 번째 confirmation read를 요구하지 않는다.
...
- 일반 개발에서는 `implementation-owner`가 기본 editable task owner로 붙고, 필요할 때만 `driver-build-fixer`, `telemetry-analyst`, `unreal-integrity-analyst`, `memory-inspection-analyst` 같은 도메인 owner가 task-graph node를 소유해 edit scope를 더 좁힌다.
- `apply_edit_proposal`, `apply_patch`, `write_file`, `replace_in_file`, scoped shell write는 node ownership/lease를 따라가며 배정된 task-owner worktree로 라우팅된다.
- `/specialists assign <node-id> <owner-profile> [glob,glob2]`로 자동 배정 대신 특정 editable task owner와 ownership을 수동 고정할 수 있다.
- `/model task-owner <owner-profile> <provider> <model> [reasoning_effort]`로 이 workspace에서 특정 task owner가 쓸 LLM을 고정하고, `/model task-owner clear <owner-profile|all>`로 override를 지울 수 있다.
- lease가 겹치지 않는 secondary edit node는 automatic editable worker가 추가 patch를 만들 수 있고, lease가 겹치면 충돌 대신 defer된다.
- parallel task-owner edit가 verification을 다시 시작하면 같은 owner나 같은 lease의 오래된 background verification bundle은 자동 supersede되고, verification-like bundle이 완료되면 owning node도 함께 닫힌다.
...
- `/new-feature`로 active feature 상태를 확인하고, `/new-feature next`로 현재 상태에 맞는 다음 안전 단계를 실행한다. native fuzz 결과가 있으면 status에서 gate로 보여준다.
- `/review plan <task>`는 여전히 one-shot 계획 검토 후 즉시 실행하는 흐름에 더 적합하다.
...
- 명령, 경로, 멘션, MCP 대상, 고정 인자, `/provider status|openai-codex-subscription|openai-codex-cli|openai-api|anthropic-claude-cli|anthropic-api|deepseek|openrouter|opencode|opencode-go|ollama|lmstudio|vllm|llama.cpp` 같은 provider 하위 명령, analyze-project mode, compact fuzz campaign action, `/create-driver-poc <driver-name>`, `/find-root-cause`, `/root-cause-patterns list|match|github-search|normalize|validate`, `/resume`, `/memory show`, `/evidence show`, `/investigate show`, `/simulate show`, `/fuzz-campaign run|show`, `/new-feature next|list`, `/session jobs status|check|bundle|cancel|cancel-bundle`, `/specialists status|assign|cleanup`, `/worktree status|list|create|enter|attach|leave|cleanup` 같은 저장된 id와 하위 명령까지 `Tab` 완성
...
- 직접 `!` shell 명령은 출력 전에 ok/failed 상태, 가능하면 exit code, elapsed time, 출력 줄 수, command preview를 한 줄 요약으로 보여준다. 실패한 명령은 첫 유효 error 줄과 짧은 next action도 함께 표시한다. 모델이 실행한 `run_shell` 완료 요약도 첫 의미 있는 출력 줄 앞에 전체 출력 줄 수를 함께 표시한다. 출력이 너무 길면 짧은 출력은 그대로 두고, 긴 출력은 header에 `collapsed`를 표시한 뒤 앞부분과 끝부분 preview, 생략 줄/문자 수 marker를 함께 보여준다. 한국어 locale에서는 collapse marker도 한국어로 출력된다.
...
대화형 provider 선택기와 `/model cross-review`는 사용자에게 다음 순서로 표시한다: `openai-codex-subscription`, `openai-codex-cli`, `openai-api`, `anthropic-claude-cli`, `anthropic-api`, `DeepSeek`, `openrouter`, `OpenCode Zen`, `OpenCode Go`, `ollama`, `LM Studio`, `vLLM`, `llama.cpp`. analysis worker, analysis reviewer, cross-review 모델 선택기는 provider 목록 앞에 `0`을 보여주며, 이 값을 선택하면 해당 target을 기본 route로 초기화한다.
...
4. 필요하면 `/memory search category:driver`로 이전 세션 맥락까지 확인합니다.
...
4. `/memory search category:telemetry tag:provider`로 과거 세션의 판단과 회귀 맥락을 다시 봅니다.
5. 이후 push/PR 전에 hook이 추가 review context나 확인을 요구할 수 있습니다.

### Task Ownership Profile과 Worktree Isolation 활용 예시

`task_ownership`은 task ownership profile의 현재 설정 키입니다. `/specialists` 명령 namespace와 legacy `specialists` 설정 키는 호환용으로 계속 동작합니다. task ownership은 기본적으로 켜져 있고, `worktree_isolation`은 기본적으로 꺼져 있습니다. 아래처럼 워크스페이스 설정을 켜 두면 tracked feature 구현, driver/telemetry/Unreal/memory 계열 고위험 수정, 그리고 서로 다른 ownership을 가진 다중 편집 요청에서 특히 효과가 큽니다.

보통의 웹, 백엔드, 툴링, 앱 기능 개발에서는 `implementation-owner`와 `planner`가 먼저 붙고, 실제 review gate는 active main model과 optional `/model cross-review` route가 담당합니다. driver/telemetry/Unreal/memory 같은 domain owner는 task 설명이나 파일 경로가 강하게 맞을 때만 들어오며, security/design/false-positive 같은 리뷰 전문성은 review lens로 적용된다고 생각하면 됩니다.
...
3. 자동 배정이 너무 넓게 잡혀서 `implementation-owner` 대신 더 좁은 domain owner를 고정하고 싶을 때
...
2. `/new-feature settings page and cache invalidation cleanup`
3. `/new-feature next`
...
5. Kernforge는 task graph node별로 task owner를 고르고, 각 node에 editable ownership과 lease를 붙입니다.
6. lease가 겹치지 않는 secondary edit node가 있으면 automatic editable worker가 별도 task-owner worktree에서 추가 patch를 만들 수 있습니다.
...
8. `/session tasks`, `/specialists status`, `/worktree status`, `/verify dashboard`로 현재 routing과 verification 상태를 확인합니다.
9. 구현이 끝나고 isolated worktree가 깨끗하면 `/worktree cleanup`으로 정리합니다.

권장 사용 흐름 2: 도메인 owner를 수동 고정

1. `/session tasks`로 node id를 확인합니다.
...
4. 다시 구현 요청을 이어갑니다.
5. 이후 edit tool과 scoped shell write는 해당 node의 ownership과 task-owner worktree 안에서만 허용됩니다.
6. ownership 밖 경로를 쓰려 하면 Kernforge가 재배정 힌트를 보여 주므로, 그때 ownership glob을 넓히거나 다른 task owner로 다시 assign하면 됩니다.
...
3. `task_ownership.profiles`로 built-in owner profile에 provider/model/ownership_paths를 오버레이할 수 있습니다. 예를 들어 `telemetry-analyst`만 더 강한 모델로 올리거나, `driver-build-fixer`의 ownership을 `package/**`까지 넓히는 식입니다.

Task-owner profile overlay 구조:
...
- `/model task-owner <owner-profile> <provider> <model> [reasoning_effort]`
- `/worktree status`
- `/worktree list`
...
- `-prompt`, `-command`, `-goal`, `-goal-file`은 단발 실행 모드입니다. ambient request-cancel keyboard watcher를 설치하지 않으며, diff preview, write approval, automatic verification 같은 명시 confirmation prompt만 stdin을 소비합니다.
...
| 필드 | 설명 |
| --- | --- |
| `provider` | `openai-codex-subscription`, `openai-codex-cli`, `openai-api`, `anthropic-claude-cli`, `anthropic-api`, `DeepSeek`, `openrouter`, `opencode`, `opencode-go`, `ollama`, `lmstudio`, `vllm`, `llama.cpp`, 그리고 `openai-compatible` |
| `model` | provider에 전달할 모델 이름 |
| `base_url` | provider API base URL |
| `api_key` | API key |
| `temperature` | 모델 temperature |
| `reasoning_effort` | active main model의 선택 OpenAI Codex 및 DeepSeek reasoning effort: `minimal`, `low`, `medium`, `high`, `xhigh`; 미설정은 `undefined`로 표시. 저장된 main profile, review profile, analysis role profile, task-owner profile도 각자의 `reasoning_effort`를 가질 수 있다. DeepSeek에서는 `minimal`/`low`/`medium`/`high`가 `high`, `xhigh`가 `max`로 매핑된다. |
...
| `shell_timeout_seconds` | `run_shell` 기본 timeout 초 단위 설정. 기본값은 `900` |
...
| `review` | 공통 review harness 자동화 설정과 optional cross-review route |
| `task_ownership` | task ownership profile 사용 여부와 built-in owner profile overlay 설정 |
| `worktree_isolation` | isolated git worktree root, branch prefix, tracked feature 자동 격리 설정 |

저장된 main profile은 task-owner model override를 `profiles[].role_models.task_owners` 아래에 저장한다. legacy `profiles[].role_models.specialists`도 로드 시 계속 허용된다.

cross review route, analysis worker/reviewer, task-owner의 `base_url`은 선택값이다. route가 main model과 같은 provider를 쓰고 `base_url`을 비워 두면 main의 정규화된 endpoint를 상속하며, 다른 provider를 쓰면 route가 직접 `base_url`을 지정하지 않는 한 해당 provider 기본 endpoint를 사용한다.
...
- 인터랙티브 루프는 `/review`, 자연어 리뷰, 수정 전 repair check에서 active main model을 1차 reviewer로 사용한다. `/model cross-review`로 설정한 optional route는 이 흐름에서 2차 cross reviewer로 동작하고, pre-write review에서는 더 엄격한 필수 reviewer edit gate를 유지한다. security/design/false-positive/regression/test/final-gate 전문성은 별도 모델 role이 아니라 review lens로 적용된다.
- `/status`, `/hooks`, session dashboard는 최신 review decision을 단순 pass/fail이 아니라 운영 요약으로 보여준다. compact view에는 최신 review id, request class, classification confidence와 ambiguity, lifecycle phase, trigger, target, mode, route quality, gate verdict/action, review/repair/document/verification gate status, single-model second-pass status/cache hit, cross-review triage count, incomplete triage blocker, 남은 repair/verification/evidence obligation, final-answer correction 상태, 다음 추천 명령이 들어간다. blocker count는 code repair, reviewer route, evidence gap, verification gap, document artifact quality, final-answer completeness로 나뉘므로 cross-review noise가 primary code blocker를 가리지 않는다.
- review artifact는 single-model second-pass 전용 섹션을 렌더링한다. ran/skipped/cached 상태, reviewed path, model route, finding count, prompt/raw-output artifact ref가 보이고, skipped이면 compact reason을 남긴다. 완료된 single-model second pass도 독립 cross-review 승인이 아니라 runtime evidence로 표시한다.
- Markdown의 cross-review triage는 사람이 바로 조치할 수 있게 finding id/title, location, status, reason, required fix, fix refs, changed paths, verification refs, user action 필요 여부를 항목별로 보여준다. `needs_user_decision`이면 확인할 대상, 안전하게 바꿀 범위, 아직 바꾸면 안 되는 범위, `/session continuity continue from review` 같은 다음 명령을 같이 노출한다.
- pre-final answer correction은 lifecycle fact로 보존된다. 변경 파일 고지, review/self-review 고지, validation 고지, remaining-risk 고지, review-only findings-first/no-edit 형식, document-artifact path/quality/limitation 고지 중 무엇 때문에 최종 답변이 보정됐는지 기록하되 내부 correction prompt는 노출하지 않고, generic legacy 완료 문구만으로는 통과시키지 않는다.
...
- 공통 review model 요청은 명시 timeout policy가 없으면 bounded per-attempt timeout을 사용한다. focused/pre-write cross reviewer에는 짧은 soft timeout을 걸고, DeepSeek strict-review retry는 작게 제한하며, progress log에는 현재 review 요청, context mode, 재시도 허용량, 최대 대기 시간을 사람이 읽는 문장으로 보여준다.
- pre-write repair 시도가 다시 리뷰를 통과하지 못하면 Kernforge는 넓은 재탐색 루프를 더 돌리지 않고, 최신 리뷰 결과와 마지막 수정안을 먼저 보여준 뒤 session에 pending repair confirmation을 저장하고 `계속 수정할까요? [y=계속, n=중지]`로 묻는다. `y`만 저장된 finding/proposal 기준으로 이어가고, `n`은 중단하며, 자연어 답변은 confirmation으로 소비하지 않는다.
...
- `test_gap`은 순수 테스트/검증 보강에만 non-blocking으로 남긴다. reviewer가 production code 변경 required_fix를 `test_gap`으로 잘못 라벨링해도, Kernforge는 해당 항목을 실행 가능한 수리 의무로 repair plan에 유지한다.
- verification evidence는 현재 patch transaction을 덮을 때만 current로 본다. session의 마지막 verification이나 persisted verification history가 최신 patch보다 오래됐거나 변경 path를 덮지 않으면 review blocker로 승격하지 않고 runtime gate warning과 `/verify --full` next action으로만 표시한다. artifact를 만들 수 있는 build/test shell 명령은 diff preview와 같은 pinned confirmation 형식인 `자동 검증을 실행할까요? [y=이번만 실행, a=세션 자동 실행, n=건너뛰기, Esc=취소]`을 사용하므로, 사용자가 거절한 검증은 명확한 미검증 gap으로 남고 semantic repair loop를 강제로 만들지 않는다. 이 명령들은 Codex App식 실행 수명주기를 tool metadata에 남긴다. verification-like 명령은 `verification_status`(`pending`, `passed`, `failed`, `skipped`)와 `command_execution_status`를 기록한다. background verification 시작은 `pending`과 `verification_evidence=false`이고, exit code 0으로 완료된 background check만 `passed` evidence가 된다. 사용자가 거절한 build는 `skipped`/`declined`로 남아 pending verification을 해소하거나 성공 검증 evidence로 계산되지 않는다. 이후 같은 턴의 검증 재시도나 `latest` background poll은 새 shell/progress status를 내기 전에 `NOT_EXECUTED` tool result와 guidance로 접고, 미검증 gap만 final/status에 남긴다. 편집 후 자동 검증이 실패하면 Kernforge는 실패 증거가 현재 patch scope에 속하는지 먼저 판정한다. 명령/scope 또는 실패 라인이 변경 path를 직접 가리킬 때만 좁은 repair loop를 허용하고, ambient workspace 또는 sibling-file 실패는 unrelated project/source 수정을 시작하지 않고 검증 risk로 보고하게 한다. out-of-scope로 판정된 뒤 같은 턴에서 모델이 build/test/verification을 재시도하거나 probing하면 이것도 `NOT_EXECUTED`로 막고, 모델은 외부/환경성 blocker를 요약해야 한다. 검증 명령이 만든 build artifact 변경은 허용하지만 source/config 파일 변경은 여전히 edit review gate 밖에서 차단한다. MSBuild workspace에서는 adaptive verification이 솔루션 전체보다 변경 파일을 포함하는 가장 가까운 `.vcxproj`를 먼저 실행하고, 프로젝트가 `ProjectConfiguration`을 선언하면 선택한 `/p:Configuration=...`와 `/p:Platform=...`을 같이 넘겨 MSBuild가 지원하지 않는 `Debug|Win32` 같은 기본값으로 떨어지지 않게 한다. 전체 솔루션 빌드는 명시적 full verification에 남긴다. configured/detected verification tool path가 PowerShell에서 leading tool name을 대체할 때는 `"MSBuild.exe" "project.vcxproj"` 같은 문자열 실행이 아니라 `& "...\MSBuild.exe" "project.vcxproj" /m` 형태로 정규화한다. verification summary는 논리 명령과 실제 resolved shell command가 다르면 둘 다 보존하고, missing-tool 판정은 빌드 출력의 임의 문구가 아니라 primary executable 기준으로만 한다.
- bypass가 아닌 비대화형 `-prompt` 실행도 자동 검증 계획과 동일한 pinned confirmation label을 화면에 출력하고, pipe로 들어온 `y`/`n`/`a` 답변을 그대로 처리한다. stdin이 없거나 EOF이면 tool outage나 암묵 승인으로 보지 않고 skipped/declined verification으로 기록한다.
- 외부 verification callback 결과는 review/runtime gate freshness 판정 전에 런타임 경계에서 정규화한다. 성공한 callback report가 `GeneratedAt`, `Trigger`, `Workspace`, `ChangedPaths`를 비워 반환하면 Kernforge가 현재 automatic verification request에서 값을 채워, built-in verification과 같은 current-patch freshness 규칙으로 판단하게 한다.
- out-of-scope 자동 검증 실패 이후의 다음 모델 요청은 tool definition 없이 전송된다. 이는 Codex식 명시 terminal state 경계와 맞춰, route가 최종 답변만 만들게 하고 이전 관성으로 나온 tool call은 shell/build 승인 prompt 없이 `NOT_EXECUTED`로 닫는다. 그 최종 답변이 돌아오면 이미 out-of-scope로 분류된 외부 blocker를 이유로 post-change review나 final-answer repair gate를 다시 열지 않고, 검증 risk disclosure만 보강해 종료한다.
- 최종 답변 guidance와 sanitizer는 답변을 읽기 쉽게 유지한다. 모델은 결과를 먼저 말하고, 변경 파일/review/verification/risk를 필요한 만큼만 고지하며, 사용자-facing 답변에는 내부 runtime jargon을 그대로 복사하지 않는다. code fence 밖의 반복 문장 run은 한 번으로 접고, 한국어 문장이 붙어 출력되는 경우 sentence spacing을 보정한다.
- post-change diff review는 final-answer coding-harness 상태와 분리된다. worker causal evidence 부족 같은 coding-harness blocker는 final/completion-audit 의무로 남지만, post-change 코드 evidence나 deterministic diff-review blocker로 주입되지 않는다.
- patch transaction이 있으면 review/final/completion runtime gate는 repository 전체 dirty file이 아니라 해당 transaction의 changed path를 우선 평가한다. git write gate는 여전히 전체 dirty worktree를 본다.
...
- `/session events tail [n]`은 최근 session event를 JSONL로 출력하고, `/session events export [path]`는 dashboard, scheduler, harness, 외부 supervisor가 읽을 수 있는 local app-server style feed로 `.kernforge/events/<session-id>.jsonl`과 `.kernforge/events/latest.jsonl`을 저장한다.
- `/session continuity [note]`는 changed files, open tasks, worktree, active failure repair, 최신 verification failure, 최근 shell/provider/tool error, background job, recovery action, next command를 `.kernforge/continuity/latest.md/json`으로 저장한다. 직접 실행한 `!shell` 실패도 `command_error` event로 저장되므로 로그를 다시 붙여넣지 않아도 local command failure에서 복구할 수 있다.
- `/session recover [note]`는 최신 provider/tool/command error, verification failure, active failure repair, background job, open task, next command를 모아 `.kernforge/recovery/latest.md/json`에 더 좁은 failure runbook으로 저장한다. 이제 structured diagnosis, 안정적인 failure signature, action-plan lifecycle status, execution log를 포함한다. `/session recover execute-safe [note]`는 whitelist를 통과한 `go test`, `go vet`, `go list`, `git status`, `git diff --check`, `/session jobs`, `/session continuity`, `/session audit` 같은 safe-auto recovery action만 실행한다.
- `/session audit [note]`는 completion blocker, warning, required artifact, 최신 verification, open task, background job, 최근 error, coding harness evidence를 `.kernforge/completion_audit/latest.md/json`으로 저장해 final answer가 완료/검증/산출물을 과장하지 않게 한다.
...
- secondary executor node는 자동 read-only worker뿐 아니라 automatic editable worker follow-up도 가질 수 있다. disjoint lease에서는 task owner가 별도 worktree에서 추가 patch를 만들고, 그 결과와 verification bundle 요약이 task graph에 다시 적재된다.
...
provider 설정과 model role 설정은 config 호환용 provider id와 별개로 안정적인 사용자 표시명을 사용한다. 예를 들어 `openai-codex-subscription`은 ChatGPT/Codex OAuth direct provider, `openai-codex-cli`는 설치된 `codex` 명령 bridge, `openai-api`는 OpenAI API provider, `anthropic-api`는 Anthropic API provider를 뜻한다.
...
- 사용자 표시 provider 이름: `anthropic-claude-cli`
- 설치된 `claude` 명령을 local provider bridge로 사용하며 Kernforge가 Anthropic API key를 전달하지 않는다.
- provider 설정 흐름에서 command path, 추가 인자, 사용할 모델 이름을 고른다.
- 비대화형 설정에는 `KERNFORGE_CLAUDE_CLI_PATH`, `KERNFORGE_CLAUDE_CLI_ARGS`를 사용할 수 있다.
...
- 저장되거나 복구된 OpenAI-compatible transcript는 replay 전에 정규화된다. 고아 `tool` result는 제거하고, 빠진 tool-call response는 Codex식 `aborted` synthetic tool result로 채워서 DeepSeek가 잘못된 `tool` message 순서로 후속 요청을 거부하지 않게 한다. 런타임 guidance가 모델의 tool-call batch를 의도적으로 대체하는 경로는 guidance 메시지 전에 명시적인 `NOT_EXECUTED` tool result를 기록해, 내부 tool output을 user context로 되살리지 않고 Codex식 typed call/output 경계를 유지한다.
...
- 사용자 표시 provider 이름: `openai-codex-cli`; 저장 provider id: `codex-cli`
...
- 비대화형 설정에는 `KERNFORGE_CODEX_CLI_PATH`, `KERNFORGE_CODEX_CLI_ARGS`를 사용할 수 있다.
- `codex-cli`에서는 `api_key`와 provider-key 저장값을 의도적으로 무시한다.

### OpenAI Codex Subscription

- 사용자 표시 provider 이름: `openai-codex-subscription`; 저장 provider id: `openai-codex`
...
- `/effort`로 target별 reasoning effort를 확인한다. `/effort high`는 active main model을 설정하고, `/effort analysis-worker low`, `/effort analysis-reviewer medium`, `/effort specialist <name> high`처럼 analysis/task-owner 모델도 따로 설정한다. 독립 review route effort는 `/model cross-review <provider> <model> [reasoning_effort]`에서 설정하고, `/model cross-review`는 번호 선택 기반 interactive 설정 flow를 제공한다. 미설정 상태는 `undefined`로 표시된다.
- reasoning effort는 configured model target별로 저장된다. Main profile, cross review route, analysis worker/reviewer profile, task-owner profile이 같은 provider 계열을 쓰더라도 서로 다른 값을 가질 수 있다.
- `/model`, `/provider`, route별 모델 명령이 effort 지원 모델을 선택했는데 해당 target의 effort가 `undefined`이면 Kernforge가 기본값 `low`를 저장한다. 이후 변경하거나 지우려면 `/effort`를 사용한다.
...
Kernforge가 MCP server로 동작하고 Codex가 MCP client인 경우, 리뷰 요청은 `kernforge_review`를 사용합니다. 이 tool은 CLI `/review`와 같은 공통 review harness를 실행하고, 전달된 diff/code, file path, plan, PR context 또는 현재 workspace의 `git diff`를 수집한 뒤 structured finding, gate status, `model_plan`과 `reviewer_runs` 기반 route/lens status, `scope_discovery`, `latest_review_freshness`, `edit_proposals`, `runtime_gate_ledger`, `single_model_second_pass`, `cross_review_triage`, `review_observability`, reason/when/safety/expected_result가 붙은 action-oriented `next_commands`, `.kernforge/reviews` artifact path를 반환합니다. raw model output은 MCP 응답에 직접 dump하지 않고 artifact ref 뒤에 둡니다. 그래서 "KernForge로 리뷰해줘"가 project analysis, verification, worker/reviewer route로 잘못 빠지지 않습니다.
...
- `/session handoff`는 changed files, open tasks, verification, recent events, artifact refs, continuation prompt를 `.kernforge/handoff/latest.md/json`으로 저장해 다른 agent나 cloud task로 넘길 수 있게 한다. `/session handoff import <path>`는 돌아온 result packet을 정규화하고 TaskGraph의 matching `completed_tasks`를 completed로 표시한다.
...
- `/session continuity`는 local resume/recovery packet을 `.kernforge/continuity/latest.md/json`으로 저장하고, `/session recover`는 `.kernforge/recovery/latest.md/json`에 좁은 failure runbook을 남기며, `/session jobs`는 저장된 background shell work를 터미널에서 직접 polling/cancel할 수 있게 한다.
- `/session audit`는 blocker, warning, verification, task, job, artifact evidence를 `.kernforge/completion_audit/latest.md/json`으로 저장하는 local final-readiness gate다.
...
/session handoff import <path>
/session dashboard --html
/session events [tail|export]
/session continuity [note]
/session recover [note]
/session audit [note]
/session jobs [status|check|bundle|cancel|cancel-bundle]
/suggest
...
/model cross-review [provider] [model] [reasoning_effort] | /model cross-review status | /model clear cross-review
...
- `/model`을 인자 없이 실행하면 현재 모델 라우팅을 먼저 보여준 뒤, interactive 모드에서는 변경할 기능 하나를 고르게 한다. analysis worker, analysis reviewer, cross-review의 provider 선택기에서 `0`을 누르면 해당 target만 inherited/default route로 초기화된다. 스크립트형 초기화는 `/model analysis-worker 0`, `/model analysis-reviewer 0`, `/model cross-review 0` 또는 `/model clear cross-review`를 사용한다.
- `/model`은 main model, analysis worker/reviewer, 선택적 task-owner model override를 바꾸는 대표 창구다. 독립 second-pass가 필요할 때만 `/model cross-review`로 공통 review harness cross route를 설정한다.
- `/effort`는 의도적으로 `/model`과 분리되어 있다. 인자 없이 실행하면 각 model target 값을 출력하고, `/effort undefined`는 active main model override를 지운다. `/effort analysis-worker high`나 `/effort specialist <name> low`처럼 analysis/task-owner 모델도 따로 바꿀 수 있다. active main provider가 reasoning effort를 지원하면 입력 프롬프트에도 `effort=<current>`가 표시된다.
...
- main model만 바꾸면 primary review route도 같이 바뀐다. optional cross route, analysis worker/reviewer route, task-owner route의 명시적 override는 유지된다. project analysis가 전용 worker/reviewer route 대신 main model을 다시 따르길 원하면 `/model analysis clear`를 실행한다. 한쪽만 초기화하려면 `/model analysis-worker 0` 또는 `/model analysis-reviewer 0`을 사용한다.
- `/profile`은 one-shot 모드에서는 프로필 목록만 보여주며 상태를 바꾸지 않는다. 저장된 main profile이 없고 현재 선택된 provider/model이 있으면 현재 설정을 첫 profile로 저장한 뒤 보여준다. Main profile은 analysis worker/reviewer와 선택적 task-owner model override set도 함께 저장한다. `/model`로 route model을 바꾸면 현재 main profile이 그 구성을 기억하고, 해당 profile을 활성화하면 전체 model set이 복원된다. 활성화, rename, delete, pin, unpin은 번호나 action을 명시해야 한다.
- 사용자 전역 profile과 workspace profile은 로드 시 병합되며, profile 배열이 빠진 설정 저장이 발생해도 기존 main profile 목록을 삭제하지 않고 보존한다.
- `/model cross-review`는 optional `cross` review route의 단일 설정 경로다. `design`, `security`, `false_positive`, `regression`, `test`, `final_gate`는 planner가 선택하는 review lens이며 모델 route가 아니다.
- `/model cross-review 0`은 optional cross route를 지우고 기본 single-model review route로 되돌린다.
- `/review-soak --mode scripted`는 `.kernforge/soak/<timestamp>/` 아래에 `document_artifact`, `review_only`, `implementation`, `modify_then_review`, `fix_from_review`, `analysis`, `mixed_flow`를 single-model/cross-model route로 검증한 artifact를 남긴다. `/review-soak --mode real-provider`는 `KERNFORGE_REAL_PROVIDER_SOAK=1`일 때만 실제 provider를 호출하며 `KERNFORGE_REAL_PROVIDER`, `KERNFORGE_REAL_MODEL`, `KERNFORGE_REAL_BASE_URL`, `KERNFORGE_REAL_API_KEY`를 우선 사용한다. 설정이 빠져 있으면 테스트 실패 대신 명확한 skipped report를 쓴다.
- `/hooks`도 compact runtime gate summary를 출력하므로 hook/policy 확인과 `/status`가 같은 freshness 및 next-command 표현을 사용한다.
...
`cmd/kernforge` 패키지는 통합형 테스트가 많아서 plain `go test ./cmd/kernforge -count=1 -timeout 15m`은 패키지가 끝나기 전까지 조용해 보일 수 있다. timeout을 진단할 때는 `-json` 또는 `-v`로 현재 실행 중인 테스트 이름을 노출하고, review/runtime-gate, agent/edit-loop, shell/verification, project-analysis cluster로 나눠 재현한다. 2026-05-28 진단에서는 `-json -timeout 2m` 재현의 마지막 active cluster가 project analysis였고, 실제 원인은 앞쪽 agent/review 테스트 누적 시간이 짧은 timeout budget을 대부분 소비한 것이었다.
...
직접 `!` shell 명령은 출력 전에 `shell` activity summary를 먼저 보여준다. 이 summary에는 ok/failed 상태, 가능하면 exit code, elapsed time, 출력 줄 수, 짧은 command preview가 들어간다. 실패한 명령은 `error=...`와 `next=...`도 표시해서 전체 출력으로 내려가기 전에 복구 방향을 볼 수 있다. 모델이 실행한 `run_shell` 완료 요약도 첫 의미 있는 출력 줄 앞에 전체 출력 줄 수를 표시한다. 긴 직접 shell output은 터미널 transcript에서 처음 80줄과 마지막 20줄 preview로 접힌다. 매우 긴 단일 라인은 문자 budget 기준으로 접으며, header의 `collapsed`와 본문 marker가 생략된 줄 또는 문자 수를 보여주므로 log가 prompt나 review 상태를 밀어내지 않는다. 한국어 locale에서는 collapse marker도 한국어로 표시된다.
...
- symbol, edge, build-context, security-overlay, derived graph fingerprint를 shard reuse 판단에 반영해 변경 영향이 symbol neighborhood 중심으로 좁혀진다.
- evidence packet을 required/supporting/ambiguous/gap으로 구분하고, required packet id를 worker prompt와 preflight contract에 노출
- model reviewer가 skipped인 single-model run에서도 deterministic claim verifier를 항상 실행하고, packet/line/symbol/graph/security invariant가 맞지 않는 high-confidence claim은 downgrade 또는 blocking으로 남긴다.
- Windows driver/IOCTL/callback/handle/memory/RPC/telemetry와 Unreal RPC/replication/asset/config/integrity boundary를 security/anti-cheat overlay로 생성
- Markdown과 JSON 분석 산출물 생성
- `FINAL_REPORT.md`, `ARCHITECTURE.md`, `SECURITY_SURFACE.md`, `API_AND_ENTRYPOINTS.md`, `BUILD_AND_ARTIFACTS.md`, `COVERAGE_LEDGER.md`, `STRUCTURAL_INDEX.md`, `EVIDENCE_PACKETS.md`, `EVIDENCE_GRAPH.md`, `SECURITY_OVERLAY.md`, `UNSUPPORTED_CLAIMS.md`, `VERIFICATION_MATRIX.md`, `FUZZ_TARGETS.md`, `OPERATIONS_RUNBOOK.md`로 구성된 운영 문서 세트 생성
...
3. `/docs-refresh`로 문서를 재생성하거나 `EVIDENCE_GRAPH.md`, `SECURITY_OVERLAY.md`, `UNSUPPORTED_CLAIMS.md`를 확인해 claim과 boundary 상태를 점검합니다.
4. verifier나 overlay가 follow-through를 남기면 `/verify`, `/simulate stealth-surface`, `/fuzz-campaign run`으로 이어갑니다.
