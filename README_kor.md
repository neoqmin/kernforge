...
- 기본적으로 `/fuzz-func`는 매칭되는 `/source-scan` 후보를 재사용하고, 없으면 target과 reachable file만 focused source-scan으로 훑은 뒤 plan에 연결한다. 필요하면 `--source-scan off`, `--source-scan focused`, `--source-scan full`, `--no-source-scan`으로 제어한다.
- 먼저 `/source-scan run`으로 source candidate를 저장한 뒤, 특정 후보를 명시적으로 이어가고 싶으면 `/fuzz-func --from-candidate <candidate-id>`를 사용한다.
...
- function fuzz run에는 `source_candidate_id`, `source_matcher_slug`, `source_scan_mode`, `source_scan_run_id`, `source_scan_summary`가 남아 campaign과 MCP feedback이 원래 source signal로 되돌아갈 수 있다.
