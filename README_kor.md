...
보통의 웹, 백엔드, 툴링, 앱 기능 개발에서는 `implementation-owner`와 `planner`가 먼저 붙고, 실제 review gate와 reviewer model은 `/review models`가 담당합니다. driver/telemetry/Unreal/memory 같은 specialist는 task 설명이나 파일 경로가 강하게 맞을 때만 들어온다고 생각하면 됩니다.
...
| `review` | 공통 review harness 자동화 설정과 role별 reviewer 모델 |
...
공통 review role, analysis worker/reviewer, specialist의 `base_url`은 선택값이다. role이 main model과 같은 provider를 쓰고 `base_url`을 비워 두면 main의 정규화된 endpoint를 상속하며, 다른 provider를 쓰면 role이 직접 `base_url`을 지정하지 않는 한 해당 provider 기본 endpoint를 사용한다.
...
- `/effort`로 target별 reasoning effort를 확인한다. `/effort high`는 active main model을 설정하고, `/effort analysis-worker low`, `/effort analysis-reviewer medium`, `/effort specialist <name> high`처럼 analysis/specialist 모델도 따로 설정한다. 공통 review role effort는 `/review models <role> <provider> <model> [reasoning_effort]`에서 설정하고, `/review models`는 번호 선택 기반 interactive 설정 flow를 제공한다. 미설정 상태는 `undefined`로 표시된다.
- reasoning effort는 configured model target별로 저장된다. Main profile, common review role model, analysis worker/reviewer profile, specialist profile이 같은 provider 계열을 쓰더라도 서로 다른 값을 가질 수 있다.
...
- `/model`은 main 모델, analysis worker/reviewer, specialist subagent 모델 오버라이드를 보여주고, 바꾸고 싶은 대상을 골라 해당 설정 흐름으로 들어가는 모델 라우팅 허브다. 공통 review harness reviewer는 `/review models`에서 설정한다.
...
- `/model`은 main model, analysis worker/reviewer, specialist subagent 모델을 바꾸는 대표 창구다. 공통 review harness reviewer role은 `/review models`에서 설정한다.
...
- `/review models`가 `primary`, `design`, `security`, `false-positive`, `regression`, `test`, `final` 같은 공통 review harness reviewer role의 단일 설정 경로다.
