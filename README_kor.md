...
- `/review`는 plan, code, selection, PR, goal, final answer, analysis report, 수정 전, 쓰기 전, 변경 후, MCP review를 같은 기반에서 처리하는 evidence-backed review harness다. structured finding, typed state transition, action envelope, approval ledger, capability manifest, freshness, next command, repair guidance를 함께 관리한다. 자연어 리뷰/수정 요청은 먼저 intent를 분류하고, 단일 모델 모드에서는 별도 staged self-review/validation 계약을 주입하며, cross reviewer가 있으면 독립 피드백을 triage하도록 repair handoff를 강화한다.
...
- 최신 review transaction, patch transaction, verification report, completion audit, final-answer review를 하나로 묶는 runtime gate ledger. `/status`, `/hooks`, session dashboard, review artifact, MCP 응답, final-answer prompt에서 review freshness, blocker, waiver, next command를 같은 의미로 보여준다.
