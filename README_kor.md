...
`Kernforge`는 Go로 작성된 터미널 중심 AI 코딩 CLI입니다. 로컬 우선 개발 흐름에 맞춰 설계되었고, 특히 Windows 개발 환경을 강하게 지원합니다.
...
- 로컬 파일 언급, 이미지 언급, MCP 리소스 언급
- 세션 저장, 재개, 이름 변경, 초기화, compact, Markdown 내보내기
- 프로젝트 메모리 파일과 세션 간 persistent memory
- 로컬 `SKILL.md` 스킬 로딩 및 요청 단위 활성화
- stdio 기반 MCP 서버의 tool/resource/prompt 연결
- Windows용 별도 텍스트 뷰어와 diff preview 창
- adaptive verification, 검증 대시보드, 체크포인트, 롤백
...
- `@main.go` 같은 파일 언급
- `@main.go:120-150` 같은 라인 범위 언급
- `@mcp:docs:getting-started` 같은 MCP 리소스 언급
- 줄 끝에 `\`를 붙여 멀티라인 입력
...
- 한 요청의 첫 편집 전에 자동 체크포인트 생성
- 수동 체크포인트, 체크포인트 diff, 롤백
- `/open`을 중심으로 한 selection-first 리뷰/수정 흐름
...
| `auto_compact_chars` | 자동 compact를 시도할 대략적인 컨텍스트 길이 |
| `auto_checkpoint_edits` | 첫 편집 전에 안전 체크포인트 생성 |
...
| `profiles` | 최근/고정 provider-model profile |
