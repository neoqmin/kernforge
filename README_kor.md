...
- `/set-specialist-model <owner-profile> <provider> [model]`로 이 workspace에서 특정 task owner가 쓸 LLM을 고정하고, `/set-specialist-model clear <owner-profile|all>`로 override를 지울 수 있다.
...
3. `task_ownership.profiles`로 built-in owner profile에 provider/model/ownership_paths를 오버레이할 수 있습니다. legacy `specialists.profiles`도 계속 허용됩니다. 예를 들어 `telemetry-analyst`만 더 강한 모델로 올리거나, `driver-build-fixer`의 ownership을 `package/**`까지 넓히는 식입니다.
...
| `task_ownership` | task ownership profile 사용 여부와 built-in owner profile overlay 설정. legacy `specialists`는 호환 alias로 계속 허용된다 |
...
- Markdown의 cross-review triage는 사람이 바로 조치할 수 있게 finding id/title, location, status, reason, required fix, fix refs, changed paths, verification refs, user action 필요 여부를 항목별로 보여준다. `needs_user_decision`이면 확인할 대상, 안전하게 바꿀 범위, 아직 바꾸면 안 되는 범위, `/continuity continue from review` 같은 다음 명령을 같이 노출한다.
...
- main model만 바꾸면 primary review route도 같이 바뀐다. optional cross route, analysis worker/reviewer route, task-owner route의 명시적 override는 유지된다. project analysis가 전용 worker/reviewer route 대신 main model을 다시 따르길 원하면 `/set-analysis-models clear`를 실행한다.
...
- `/set-specialist-model ...`은 특정 task owner profile에만 workspace 단위의 선택적 모델 override를 줄 때 쓴다.
...
- `/progress-display`는 runtime 진행 표시 모드를 보여주거나 저장한다. `/progress_display`도 같은 명령으로 처리된다. 기본값은 `compact`이며 Codex CLI처럼 조용한 operator view를 우선한다. `auto`는 반복적인 상세 review flow text 없이 중요한 tool/model ledger만 남기고, `stream`은 모든 progress update를 transcript에 남긴다.
