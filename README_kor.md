...
- `/goal`, `-goal`, `-goal-file`은 긴 목표를 끝까지 밀어붙이는 autonomous execution 계층이다. interactive `/goal`은 prompt나 markdown 파일의 목표를 visible artifact와 함께 먼저 영속화하고, 명시 실행 form이 구현, 독립 review, repair, full verification, completion audit, 최종 semantic review, recovery를 목표 완료 또는 구체 blocker 기록까지 반복한다.
...
- `/goal`, `-goal`, `-goal-file` 기반 Codex식 autonomous goal. prompt 또는 markdown 목표를 먼저 기록할 수 있고, 명시 실행 시 구현, 자체 리뷰, 검증, completion audit, 최종 semantic review, recovery를 사용자 확인 없이 반복한다.
...
- 암시적 model-backed review 요청은 `review.model_review_consent`를 따른다. 기본값은 `ask`이고 `always`, `never`를 지원한다. 기존 automatic review flag는 리뷰 대상 자격만 의미하며, 명시적 `/review`, 자연어 리뷰 요청, `kernforge_review`, 명시적으로 등록한 review automation은 추가 확인 없이 명시 요청으로 처리된다. 자동 리뷰가 생략되거나 reviewer가 차단하더라도 사용자가 볼 수 있었던 최초 main-model proposal은 가능한 경우 review/session artifact에 보존된다.
...
- 그 다음 Kernforge가 `/verify --full`, `/session audit`, 최종 semantic review, 필요 시 `/session recover execute-safe`를 실행한 뒤 다음 iteration으로 넘어간다.
...
- `/model`을 인자 없이 실행하면 현재 모델 라우팅을 먼저 보여준 뒤, interactive 모드에서는 변경할 기능 하나를 고르게 한다. 스크립트형 설정은 `/model analysis-worker ...`, `/model analysis-reviewer ...`, `/model task-owner ...`를 사용한다. analysis worker/reviewer와 cross-review의 provider 선택기에서 `0`을 누르면 해당 target만 기본 route로 초기화된다.
