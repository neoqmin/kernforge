...
- `/create-driver-poc <driver-name>`으로 x64 전용 C++20 MSVC/WDK POC 드라이버 솔루션을 생성한다. 드라이버는 `Driver.cpp` 기반이고, service/device/IOCTL 이름과 `DeviceType`은 namespace/constexpr 공통 통신 헤더에 모이며, INF package는 만들지 않고, tester Release 빌드는 MT runtime으로 링크한다. 같은 출력 디렉터리의 `<driver-name>-tester.exe`는 SCM 설치/시작, 이미 실행 중인 stale 서비스 재시작, 핸들 open, IOCTL ping, unload/delete를 수행하며, tester 자신의 경로는 고정 `MAX_PATH` 대신 증가 버퍼로 해석한다.
...
- `/create-driver-poc <driver-name>`은 x64 전용 C++20 MSVC/WDK driver POC 템플릿을 생성한다. 산출물은 `Driver.cpp` 기반 WDM `.sys`, namespace/constexpr service/device/IOCTL/`DeviceType` 공통 header, INF 없는 SCM 기반 load 경로, `CreateFile`, `DeviceIoControl`을 사용하는 `<driver-name>-tester.exe`, Release MT runtime 설정까지 포함한다. tester는 이미 실행 중인 서비스를 중지/대기 후 재시작해 stale driver image 테스트를 피하고, 긴 실행 경로도 증가 버퍼로 처리한다.
