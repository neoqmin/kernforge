...
- 일반 개발에서는 `implementation-owner`가 기본 editable specialist로 붙고, 필요할 때만 `driver-build-fixer`, `telemetry-analyst`, `unreal-integrity-analyst`, `memory-inspection-analyst` 같은 도메인 specialist가 task-graph node를 소유해 edit scope를 더 좁힌다.
- `apply_edit_proposal`, `apply_patch`, `write_file`, `replace_in_file`, scoped shell write는 node ownership/lease를 따라가며 배정된 specialist worktree로 라우팅된다.
...
보통의 웹, 백엔드, 툴링, 앱 기능 개발에서는 `implementation-owner`와 `planner`가 먼저 붙고, 실제 review gate는 active main model과 optional `/review models cross` route가 담당합니다. driver/telemetry/Unreal/memory 같은 specialist는 task 설명이나 파일 경로가 강하게 맞을 때만 들어오며, security/design/false-positive 같은 리뷰 전문성은 review lens로 적용된다고 생각하면 됩니다.
...
cross review route, analysis worker/reviewer, specialist의 `base_url`은 선택값이다. route가 main model과 같은 provider를 쓰고 `base_url`을 비워 두면 main의 정규화된 endpoint를 상속하며, 다른 provider를 쓰면 route가 직접 `base_url`을 지정하지 않는 한 해당 provider 기본 endpoint를 사용한다.
...
- `/effort`로 target별 reasoning effort를 확인한다. `/effort high`는 active main model을 설정하고, `/effort analysis-worker low`, `/effort analysis-reviewer medium`, `/effort specialist <name> high`처럼 analysis/specialist 모델도 따로 설정한다. 독립 review route effort는 `/review models cross <provider> <model> [reasoning_effort]`에서 설정하고, `/review models`는 번호 선택 기반 interactive 설정 flow를 제공한다. 미설정 상태는 `undefined`로 표시된다.
- reasoning effort는 configured model target별로 저장된다. Main profile, cross review route, analysis worker/reviewer profile, specialist profile이 같은 provider 계열을 쓰더라도 서로 다른 값을 가질 수 있다.
...
- `/model`은 main 모델, analysis worker/reviewer, specialist subagent 모델 오버라이드를 보여주고, 바꾸고 싶은 대상을 골라 해당 설정 흐름으로 들어가는 모델 라우팅 허브다. primary review route는 main model을 따르고, optional cross review route만 `/review models`에서 설정한다.
...
- `/model`은 main model, analysis worker/reviewer, specialist subagent 모델을 바꾸는 대표 창구다. 독립 second-pass가 필요할 때만 `/review models cross`로 공통 review harness cross route를 설정한다.
- `/effort`는 의도적으로 `/model`과 분리되어 있다. 인자 없이 실행하면 각 model target 값을 출력하고, `/effort undefined`는 active main model override를 지운다. `/effort analysis-worker high`나 `/effort specialist <name> low`처럼 analysis/specialist 모델도 따로 바꿀 수 있다. active main provider가 reasoning effort를 지원하면 입력 프롬프트에도 `effort=<current>`가 표시된다.
...
- main model만 바꾸면 primary review route도 같이 바뀐다. optional cross route, analysis worker/reviewer route, specialist route의 명시적 override는 유지된다. project analysis가 전용 worker/reviewer route 대신 main model을 다시 따르길 원하면 `/set-analysis-models clear`를 실행한다.
- `/profile`은 one-shot 모드에서는 프로필 목록만 보여주며 상태를 바꾸지 않는다. 저장된 main profile이 없고 현재 선택된 provider/model이 있으면 현재 설정을 첫 profile로 저장한 뒤 보여준다. Main profile은 analysis worker/reviewer, specialist subagent 역할별 model set도 함께 저장한다. `/model`로 역할별 model을 바꾸면 현재 main profile이 그 구성을 기억하고, 해당 profile을 활성화하면 전체 model set이 복원된다. 활성화, rename, delete, pin, unpin은 번호나 action을 명시해야 한다.
