...
대화형 provider 선택기와 `/review models`는 사용자에게 다음 순서로 표시한다: `openai-codex-subscription`, `openai-codex-cli`, `openai-api`, `anthropic-claude-cli`, `anthropic-api`, `DeepSeek`, `openrouter`, `OpenCode Zen`, `OpenCode Go`, `ollama`, `LM Studio`, `vLLM`, `llama.cpp`.
...
보통의 웹, 백엔드, 툴링, 앱 기능 개발에서는 `implementation-owner`와 `planner`가 먼저 붙고, 실제 review gate는 active main model과 optional `/review models cross` route가 담당합니다. driver/telemetry/Unreal/memory 같은 domain owner는 task 설명이나 파일 경로가 강하게 맞을 때만 들어오며, security/design/false-positive 같은 리뷰 전문성은 review lens로 적용된다고 생각하면 됩니다.
...
- 인터랙티브 루프는 `/review`, 자연어 리뷰, 수정 전 repair check에서 active main model을 1차 reviewer로 사용한다. `/review models cross`로 설정한 optional route는 이 흐름에서 2차 cross reviewer로 동작하고, pre-write review에서는 더 엄격한 필수 reviewer edit gate를 유지한다. security/design/false-positive/regression/test/final-gate 전문성은 별도 모델 role이 아니라 review lens로 적용된다.
...
- `/effort`로 target별 reasoning effort를 확인한다. `/effort high`는 active main model을 설정하고, `/effort analysis-worker low`, `/effort analysis-reviewer medium`, `/effort specialist <name> high`처럼 analysis/task-owner 모델도 따로 설정한다. 독립 review route effort는 `/review models cross <provider> <model> [reasoning_effort]`에서 설정하고, `/review models`는 번호 선택 기반 interactive 설정 flow를 제공한다. 미설정 상태는 `undefined`로 표시된다.
...
- `/model`은 main 모델, analysis worker/reviewer, 선택적 task-owner 모델 override를 보여주고, 바꾸고 싶은 대상을 골라 해당 설정 흐름으로 들어가는 모델 라우팅 허브다. primary review route는 main model을 따르고, optional cross review route만 `/review models`에서 설정한다.
...
/review models [cross|status|clear]
/new-feature <task>
/specialists
...
- `/model`은 main model, analysis worker/reviewer, 선택적 task-owner model override를 바꾸는 대표 창구다. 독립 second-pass가 필요할 때만 `/review models cross`로 공통 review harness cross route를 설정한다.
...
- `/review models`는 optional `cross` review route의 단일 설정 경로다. `design`, `security`, `false_positive`, `regression`, `test`, `final_gate`는 planner가 선택하는 review lens이며 모델 route가 아니다.
