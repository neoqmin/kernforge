...
- `/review`는 plan, code, selection, PR, goal, final answer, analysis report, 수정 전, 쓰기 전, 변경 후, MCP review를 같은 기반에서 처리하는 evidence-backed review harness다. structured finding, request class, lifecycle phase, route mode, action envelope, approval ledger, capability manifest, freshness, next command, repair guidance를 함께 관리한다. 자연어 리뷰/수정 요청은 `review_only`, `document_artifact`, `review_then_modify`, `modify_then_review`, `verification_only`, `validation_only` 같은 lifecycle로 먼저 분류된다. 단순 리뷰는 read-only와 findings-first를 유지하고, 문서 artifact 요청은 code review loop가 아니라 artifact-quality gate를 우선한다. 단일 모델 모드에서는 별도 second-pass review 상태를 실행하거나 명시적으로 기록하며, cross reviewer가 있으면 독립 피드백을 `cross_review_triage`로 조정하도록 repair handoff를 강화한다.
...
- 최신 review transaction, patch transaction, verification report, completion audit, final-answer review를 하나로 묶는 runtime gate ledger. `/status`, `/hooks`, session dashboard, review artifact, MCP 응답, final-answer prompt에서 request class, lifecycle phase, single-model/cross-model route mode, review/repair/document/verification gate status, second-pass state, cross-review triage count, blocker, waiver, obligation, next command를 같은 의미로 보여준다.
...
- plan, code, selection, PR, goal, final, analysis, 수정 전, 쓰기 전, 변경 후, MCP review를 같은 기반에서 처리하는 공통 `/review` 하네스. main-first 리뷰, 선택적 cross reviewer, request-class-aware orchestration, typed action envelope, 분리된 approval ledger, route health 기반 retry 억제, 로컬 코드 수리 중 웹 검색 차단, pre-write repair 실패 후 명시적 `y/N` continuation, artifact integrity 검사, replay fixture, diff preview 전 최종 리뷰 본문 출력을 포함한다. 자연어 리뷰/수정/goal 실행에는 Codex-grade 요청 처리 계약을 주입하고, 이제 핵심 계약은 runtime에서 강제된다. 요청은 `review_only`, `document_artifact`, `review_then_modify`, `modify_then_review`, `verification_only`, `validation_only`, `general`로 분류되고, 그 이유가 `ReviewRun`, runtime gate, status/artifact, MCP 응답에 남는다. distinct cross route가 없으면 별도 single-model second-pass review call을 실행하거나 skipped reason을 명시하고, actionable cross-review finding은 `cross_review_triage`에서 `accepted_fixed`, `accepted_deferred`, `rejected_with_reason`, `needs_user_decision` 중 하나로 조정해야 한다. 문서 artifact는 artifact 존재, 요청 topic coverage, placeholder/TODO-only 여부, unsupported verification claim 여부를 기준으로 판단해 code review나 shell verification으로 과차단하지 않는다. 수정/리뷰 final answer가 변경 파일, review 결과, validation 결과, remaining risk를 빠뜨리면 pre-final coding harness가 내부 수정 턴으로 막는다. UI-polish review도 changed path가 style/asset/markup-only임을 증명하지 못하면 primary reviewer coverage를 함께 요구하며, 실행 가능한 source 파일은 이름이 UI처럼 보여도 primary correctness coverage를 유지한다.
...
- `/status`, `/hooks`, session dashboard는 최신 review decision을 단순 pass/fail이 아니라 운영 요약으로 보여준다. compact view에는 최신 review id, trigger, target, mode, gate verdict/action, single-model second-pass status/cache hit, cross-review triage count, incomplete triage blocker, 남은 repair/verification/evidence obligation, final-answer correction 상태, 다음 추천 명령이 들어간다. blocker count는 code repair, reviewer route, evidence gap, verification gap, final-answer completeness로 나뉘므로 cross-review noise가 primary code blocker를 가리지 않는다.
...
- Markdown의 cross-review triage는 사람이 바로 조치할 수 있게 finding id/title, location, status, reason, required fix, fix refs, changed paths, verification refs, user action 필요 여부를 항목별로 보여준다. `needs_user_decision`이면 `/continuity continue from review` 같은 구체적인 continuation prompt를 노출한다.
- pre-final answer correction은 lifecycle fact로 보존된다. 변경 파일 고지, review/self-review 고지, validation 고지, remaining-risk 고지, review-only findings-first/no-edit 형식 중 무엇 때문에 최종 답변이 보정됐는지 기록하되 내부 correction prompt는 노출하지 않는다.
