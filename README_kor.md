...
- worker와 reviewer가 같은 provider/model route를 공유하는 단일 모델 환경에서는 기본 shard concurrency를 자동으로 1로 낮춰 provider 재시도 실패와 저신뢰 placeholder 폭증을 줄인다. 사용자가 agent cap을 명시한 경우에는 그 설정을 우선한다.
...
| `reasoning_effort` | 선택 OpenAI Codex reasoning effort: `minimal`, `low`, `medium`, `high`, `xhigh`; 미설정은 `undefined`로 표시 |
...
| `model_routes` | provider/model/base_url/reasoning_effort route별 동시 모델 요청 제한. local provider와 단일 모델 route는 기본 직렬화해 rate-limit, local server 포화, shard placeholder 폭증을 줄인다. |
...
- `/effort`로 현재 reasoning effort를 확인하고, `/effort high`처럼 값을 지정해 설정한다. 미설정 상태는 `undefined`로 표시된다.
- reasoning effort는 main model뿐 아니라 plan reviewer, analysis worker/reviewer, specialist subagent가 `openai-codex`를 사용할 때 해당 Responses 요청 전체에 적용된다.
- `/model`은 reasoning effort를 읽거나 저장하지 않는다. effort 설정은 `/effort`에서만 다룬다.
...
- `/effort`는 `/model`과 별개로 `openai-codex` reasoning effort를 보여주거나 설정한다.
...
- `/effort`는 의도적으로 `/model`과 분리되어 있다. 인자 없이 실행하면 현재 값을 출력하고, `/effort undefined`는 override를 지운다.
