...
- 긴 분석 실행 중에도 총 shard 수, worker slot 수, wave 진행, shard 완료 상태, worker/reviewer 모델 대기 또는 실패 event, 마지막 artifact 저장 단계가 일반 progress 표시 정책을 통해 드러난다.
...
Kernforge가 MCP server로 동작하고 Codex가 MCP client인 경우, 리뷰 요청은 `kernforge_review`를 사용합니다. 이 tool은 CLI `/review`와 같은 공통 review harness를 실행하고, 전달된 diff/code, file path, plan, PR context 또는 현재 workspace의 `git diff`를 수집한 뒤 structured finding, gate status, model-role status, reason/when/safety가 붙은 action-oriented `next_commands`, `.kernforge/reviews` artifact path를 반환합니다. 그래서 "KernForge로 리뷰해줘"가 project analysis, verification, worker/reviewer route로 잘못 빠지지 않습니다.
