...
- `progress_display`가 진행 표시 방식을 제어하며 기본값은 긴 작업 디버깅을 위해 모든 progress update를 본문에 지속 기록하는 `stream`이다. REPL에서는 `/progress-display auto|compact|stream`으로 바로 바꿀 수 있고, config key를 그대로 친 `/progress_display ...`도 같은 명령으로 처리한다. `auto`는 중요한 tool/model/route와 project analysis 진행 event를 transcript ledger로 남기고, `compact`는 footer 중심으로 조용하게 보여주며, `stream`은 모든 progress update를 본문에 지속 기록한다.
...
| `progress_display` | runtime 진행 표시 방식. 기본값 `stream`은 긴 작업 디버깅을 위해 모든 progress update를 transcript에 기록한다. `auto`는 중요한 tool/model/project-analysis ledger는 본문에 남기고 고빈도 shell 출력은 footer에 두며, `compact`는 진행 상태를 transient footer 중심으로 유지한다. |
...
- `/progress-display`는 runtime 진행 표시 모드를 보여주거나 저장한다. `/progress_display`도 같은 명령으로 처리된다. 기본값은 `stream`이며 모든 progress update를 transcript에 남긴다. `auto`는 중요한 tool/model ledger는 본문에 남기고 noisy output은 transient 처리하며, `compact`는 footer 중심으로 진행 상태를 보여준다.
